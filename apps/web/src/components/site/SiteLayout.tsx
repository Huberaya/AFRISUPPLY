    ...
    <div className="min-h-screen bg-white text-stone-900">
      <header className="sticky top-0 z-40 border-b border-stone-100 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 lg:px-8">
          ...
          <div className="hidden items-center gap-2 md:flex">{CartBtn}<HomeButton />{user ? <Link to="/app" className="btn-primary">Mon espace</Link> : <><Link to="/connexion" className="btn-ghost">Se connecter</Link><Link to="/inscription" className="btn-primary">Créer un compte</Link></>}</div>
          <div className="flex items-center md:hidden">{CartBtn}<HomeButton /><button className="rounded-lg p-2" onClick={() => setOpen(!open)} aria-label="Menu">{open ? <X /> : <Menu />}</button></div>
        </div>
        {open && <div className="border-t border-stone-100 px-4 py-3 md:hidden flex flex-col gap-2 text-sm"><div className="flex" onClick={() => setOpen(false)}><HomeButton /></div>{NAV.map((n) => <Link key={n.to} to={n.to} onClick={() => setOpen(false)} className="py-1.5">{n.label}</Link>)}{user ? <Link to="/app" onClick={() => setOpen(false)} className="btn-primary justify-center">Mon espace</Link> : <><Link to="/connexion" onClick={() => setOpen(false)} className="py-1.5">Se connecter</Link><Link to="/inscription" onClick={() => setOpen(false)} className="btn-primary justify-center">Créer un compte</Link></>}</div>}
      </header>
      <main><Outlet /></main>
