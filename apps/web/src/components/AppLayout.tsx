    ...
    <aside className="flex h-full w-64 flex-col bg-stone-900 text-stone-200">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        {user?.isAdmin && <>
          ...
          {[['/app/admin/pilotes', 'Cockpit pilotes'], ['/app/admin/prospection', 'Prospection'], ['/app/admin/abonnements', 'Abonnements'], ['/app/admin/fournisseurs', 'Fournisseurs plateforme'], ['/app/admin/referentiel', 'Référentiel produits']].map(([to, label]) => <NavLink key={to} to={to} onClick={() => setOpen(false)} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition ${isActive ? 'bg-brand-700 text-white' : 'hover:bg-stone-800 text-stone-300'}`}><ShieldCheck size={18} /> {label}</NavLink>)}
        </>}
      </nav>
