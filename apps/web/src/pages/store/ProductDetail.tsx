...
export default function ProductDetail() {
  const { id } = useParams(); const { data, loading, error } = useApi<Data>(`/public/products/${id}`);
  ...
  if (loading) return <div className="p-10"><Loader /></div>; if (error) return <div className="p-10"><ErrorBox message={error} /></div>; if (!data) return null;
  ...
  const alert = async () => { try { const r = await api<{ message: string }>('/public/product-alert', { method: 'POST', json: { productId: p.id, email } }); setAlertMsg(r.message); } catch (e) { setAlertMsg((e as Error).message); } };
  return (
