...
export function Propose({ orderId, lines, onDone, onCancel }: { orderId: string; lines: Line[]; onDone: () => void; onCancel: () => void }) {
  ...
  const send = async () => { setBusy(true); setErr(null); try { await api(`/vendor/orders/${orderId}/propose`, { method: 'POST', json: { note: note || undefined, lines: lines.map((l) => ({ lineId: l.id, newPacks: q[l.id] ?? l.packs, replacementOfferId: rep[l.id] || null, replacementPacks: rep[l.id] ? Math.max(1, repPacks[l.id] ?? (l.packs - (q[l.id] ?? l.packs))) : undefined })) } }); onDone(); } catch (e) { setErr((e as Error).message); } finally { setBusy(false); } };
  return (
