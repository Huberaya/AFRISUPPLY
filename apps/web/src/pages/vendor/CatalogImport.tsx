    ...
    <div className="space-y-4">
      ...
      {lines && lines.length > 0 && (
        <div className="card space-y-3 p-0">
          ...
          <div className="flex items-center justify-between gap-2 p-4 pt-0"><p className="text-xs text-stone-500">Les lignes « inconnues » peuvent être ajoutées au référentiel : écrivez-nous, on les crée sous 24 h.</p><button className="btn-primary" disabled={busy || !ready.length} onClick={() => void publish()}><Check size={16} /> Publier {ready.length} offre(s)</button></div>
        </div>
      )}
    </div>
  );
