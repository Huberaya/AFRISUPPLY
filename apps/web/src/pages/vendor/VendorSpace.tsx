...
import { Store, Package, Inbox, Users, Receipt, Check, X, Truck, LogOut, BarChart3, FileText, Scale, Tag, Star, Route, Wallet, CreditCard } from 'lucide-react';
...
type Tab = 'dashboard' | 'offers' | 'pricing' | 'credit' | 'payments' | 'reviews' | 'routes' | 'orders' | 'fulfillment' | 'claims' | 'groupbuys' | 'commissions' | 'analytics';
...
export default function VendorSpace() {
  ...
  const [me, setMe] = useState<{ vendors: Vendor[]; isAdmin: boolean } | null>(null); const [err, setErr] = useState<string | null>(null); const [tab, setTab] = useState<Tab>((sp.get('tab') as Tab | null) ?? 'orders');
