...
type Offer = { id: string; productName: string; category: string; unit: string; packLabel: string; packQty: string; packPriceEur: string; unitPrice: number; myBestUnitPrice: number | null; savingPct: number | null; tracked: boolean; inStock: boolean };
...
function VendorDetail({ id, back }: { id: string; back: () => void }) {
  ...
  const [cart, setCart] = useState<Record<string, number>>({}); const [msg, setMsg] = useState<string | null>(null); const [busy, setBusy] = useState(false); const [onlyMine, setOnlyMine] = useState(true);
  useEffect(() => { setCart({}); }, [id]);
  if (loading) return <Loader />; if (error) return <ErrorBox message={error} />; if (!data) return null;
  const v = data.vendor; const offers = data.offers.filter((o) => !onlyMine || o.tracked);
  const total = data.offers.reduce((a, o) => a + (cart[o.id] ?? 0) * Number(o.packPriceEur), 0); const nb = Object.values(cart).filter((x) => x > 0).length;
        ...
        <table className="w-full text-sm"><thead className="bg-stone-50 text-left text-xs uppercase text-stone-500"><tr><th className="p-3">Produit</th><th className="p-3">Conditionnement</th><th className="p-3 text-right">Prix</th><th className="p-3 text-right">vs mon meilleur prix</th><th className="p-3 text-right">Colis</th></tr></thead>
          <tbody className="divide-y divide-stone-100">{offers.map((o) => (
            <tr key={o.id} className={!o.inStock ? 'opacity-50' : ''}>
              ...
              <td className="p-3 text-right"><b>{eur(o.packPriceEur)}</b><br /><span className="text-xs text-stone-500">{o.unitPrice.toFixed(2).replace('.', ',')} €/{o.unit}</span></td>
