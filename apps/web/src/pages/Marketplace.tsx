...
function VendorDetail({ id, back }: { id: string; back: () => void }) {
  ...
  const credit = useApi<{ terms: { paymentDays: number; creditLimitEur: number | null; blocked: boolean }; exposure: { outstandingEur: number; overdueEur: number; overdueCount: number } }>(`/marketplace/vendors/${id}/credit`);
  ...
  return (
    <div className="animate-fade-up space-y-4 pb-28">
      ...
      <PageTitle title={v.name} subtitle={<>{v.city ?? 'En ligne'} · livraison sous {v.leadTimeHours} h · minimum {eur(v.minOrderEur)}{v.description ? <><br />{v.description}</> : null}{data.reliability && <><br /><ReliabilityBadge r={data.reliability} detailed /></>}{credit.data && <><br /><span className="text-xs">💳 {credit.data.terms.paymentDays === 0 ? 'Paiement comptant à la livraison' : `Paiement à ${credit.data.terms.paymentDays} jours`}{credit.data.terms.creditLimitEur !== null && ` · encours ${eur(String(credit.data.exposure.outstandingEur))} / ${eur(String(credit.data.terms.creditLimitEur))}`}{credit.data.exposure.overdueCount > 0 && <span className="ml-1 font-semibold text-red-700">· {credit.data.exposure.overdueCount} facture{credit.data.exposure.overdueCount > 1 ? 's' : ''} en retard ({eur(String(credit.data.exposure.overdueEur))})</span>}{credit.data.terms.blocked && <span className="ml-1 font-semibold text-red-700">· compte bloqué</span>}</span></>}</>} action={data.linkedSupplierId ? <span className="pill bg-emerald-50 text-emerald-800"><Check size={12} /> Dans vos fournisseurs</span> : <button className="btn-ghost" disabled={busy} onClick={() => void link()}><Link2 size={16} /> Ajouter à mes fournisseurs</button>} />
