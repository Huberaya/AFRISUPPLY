...
async function measuredMonthlyUse(restaurantId, productId) {
  ...
  if (!item) return 0;
  const day = (n17) => new Date(Date.now() - n17 * 864e5).toISOString();
...
var init_seed = __esm({
  "packages/db/src/seed.ts"() {
    ...
    init_schema();
    init_products();
    daysAgo = (n17) => new Date(Date.now() - n17 * 864e5);
...
function tokenTtlSeconds(ttl = TOKEN_TTL) {
  ...
  if (!m) return 7 * 86400;
  const n17 = Number(m[1]);
  const unit2 = m[2] ?? "s";
  return n17 * ({ s: 1, m: 60, h: 3600, d: 86400 }[unit2] ?? 1);
}
...
function forecastRecipes(sales2, recipeIds, opts = {}) {
  ...
  for (const rid of recipeIds) {
    ...
    for (const [day, p] of hist) {
      const age = (today2.getTime() - new Date(day).getTime()) / DAY_MS;
...
function buildSmartCart(needs, offers) {
  ...
  const choices = [];
  let baseline = 0;
  for (const n17 of needs) {
    if (n17.neededQty <= 0) continue;
    const cands = offers.filter((o) => o.productId === n17.productId && o.inStock);
    if (!cands.length) {
      unavailable.push({ productId: n17.productId, productName: n17.productName, unit: n17.unit, neededQty: n17.neededQty });
      continue;
    }
    const urgencyH = n17.daysOfStockLeft !== null ? Math.max(24, n17.daysOfStockLeft * 24) : Infinity;
    const score = (o) => {
      const packs2 = Math.max(1, Math.ceil(n17.neededQty / o.packQty));
    ...
    const best = ranked[0];
    const packs = Math.max(1, Math.ceil(n17.neededQty / best.packQty));
    const usual = n17.preferredSupplierId ? cands.find((o) => o.supplierId === n17.preferredSupplierId) : void 0;
    const usualCost = usual ? Math.max(1, Math.ceil(n17.neededQty / usual.packQty)) * usual.packPrice : packs * best.packPrice;
    baseline += usualCost;
    const lineTotal = packs * best.packPrice;
    const reason = best.leadTimeHours > 48 && urgencyH !== Infinity ? `D\xE9lai ${Math.round(best.leadTimeHours / 24)} j accept\xE9 (stock ${n17.daysOfStockLeft} j)` : usual && usual.offerId !== best.offerId ? `${best.supplierName} moins cher que ${usual.supplierName} (${(usualCost - lineTotal).toFixed(2)} \u20AC \xE9conomis\xE9s)` : best.unitPrice === Math.min(...cands.map((c) => c.unitPrice)) ? "Meilleur prix disponible" : "Meilleur compromis prix / d\xE9lai / fiabilit\xE9";
    choices.push({ line: { productId: n17.productId, productName: n17.productName, unit: n17.unit, neededQty: n17.neededQty, offer: best, packs, quantity: packs * best.packQty, lineTotal, alternativeSaving: Math.max(0, usualCost - lineTotal), reason }, alternatives: ranked.slice(1) });
  }
  const group = () => {
...
async function termsFor(vendorId, restaurantId) {
  const db = await getDb();
  const [t] = await db.select().from(vendorCreditTerms).where(and10(eq12(vendorCreditTerms.vendorId, vendorId), eq12(vendorCreditTerms.restaurantId, restaurantId)));
  return t ?? null;
}
async function checkCredit(vendorId, restaurantId, amount, vendorName) {
  ...
  if (terms?.creditLimitEur !== null && terms?.creditLimitEur !== void 0 && exposure.outstandingEur + amount > n4(terms.creditLimitEur))
    return { ok: false, error: `Plafond d'encours d\xE9pass\xE9 chez ${vendorName} : ${eur5(exposure.outstandingEur)} en cours + ${eur5(amount)} > ${eur5(n4(terms.creditLimitEur))} autoris\xE9s.`, status: 402 };
...
async function remindPayments(now = /* @__PURE__ */ new Date()) {
  ...
  const rows = await db.select({ o: orders, vendorName: vendors2.name, restaurantName: restaurants2.name }).from(orders).innerJoin(vendors2, eq12(vendors2.id, orders.vendorId)).innerJoin(restaurants2, eq12(restaurants2.id, orders.restaurantId)).where(and10(sql7`(${orders.status} in ('livree','livree_partiel') or ${orders.vendorDeliveredAt} is not null)`, sql7`${orders.paidAt} is null`, sql7`${orders.dueAt} is not null`, sql7`${orders.paymentDays} > 0`));
  ...
  let reminded = 0;
  for (const r of due) {
    const amount = Math.max(0, n4(r.o.totalEur) + n4(r.o.deliveryFeeEur) - n4(r.o.paidAmountEur));
    if (amount <= 0) continue;
    const rcpts = await db.select({ email: users2.email }).from(restaurantMembers2).innerJoin(users2, eq12(users2.id, restaurantMembers2.userId)).where(and10(eq12(restaurantMembers2.restaurantId, r.o.restaurantId), inArray4(restaurantMembers2.role, ["owner", "manager"])));
    const late = r.d > 0;
...
var init_credit = __esm({
  "apps/api/src/lib/credit.ts"() {
    "use strict";
    init_src();
    n4 = (v) => v === null || v === void 0 ? 0 : Number(v);
...
import { and as and11, eq as eq13 } from "drizzle-orm";
...
async function connectOnboardingLink(vendorId, email) {
  const db = await getDb();
  const [v] = await db.select().from(vendors).where(eq13(vendors.id, vendorId));
  ...
  if (!acct) {
    ...
    acct = a.id;
    await db.update(vendors).set({ stripeAccountId: acct }).where(eq13(vendors.id, vendorId));
  }
...
async function syncConnectAccount(vendorId) {
  ...
  await db.update(vendors).set({ stripePayoutsEnabled: !!a.payouts_enabled && !!a.charges_enabled }).where(eq13(vendors.id, vendorId));
...
async function createOrderCheckout(orderId, restaurantId, userEmail) {
  const db = await getDb();
  const [o] = await db.select().from(orders).where(and11(eq13(orders.id, orderId), eq13(orders.restaurantId, restaurantId)));
  if (!o?.vendorId) throw Object.assign(new Error("Commande plateforme introuvable"), { status: 404 });
  const [v] = await db.select().from(vendors).where(eq13(vendors.id, o.vendorId));
  const [r] = await db.select({ name: restaurants.name }).from(restaurants).where(eq13(restaurants.id, restaurantId));
  ...
  const fee = Math.round(cents(amount) * n5(v.commissionPct) / 100);
  ...
  }, { idempotencyKey: `chk-${o.id}-${cents(amount)}-${n5(o.paidAmountEur)}` });
  await db.update(orders).set({ stripeCheckoutId: s.id }).where(eq13(orders.id, o.id));
...
async function applyOrderPayment(session) {
  ...
  const db = await getDb();
  const [o] = await db.select().from(orders).where(eq13(orders.id, orderId));
  if (!o || o.paidAt) return orderId;
  const paidNow = (session.amount_total ?? 0) / 100;
  const total = Math.round((n5(o.totalEur) + n5(o.deliveryFeeEur)) * 100) / 100;
  const newPaid = Math.min(total, Math.round((n5(o.paidAmountEur) + paidNow) * 100) / 100);
  await db.update(orders).set({ paidAmountEur: newPaid.toFixed(2), paidAt: newPaid >= total - 5e-3 ? /* @__PURE__ */ new Date() : null, paymentMethod: "en_ligne", stripePaymentIntentId: typeof session.payment_intent === "string" ? session.payment_intent : o.stripePaymentIntentId }).where(eq13(orders.id, o.id));
  void logOrderEvent(o.id, "note", `\u{1F4B3} Paiement en ligne re\xE7u : ${paidNow.toFixed(2)} \u20AC (Stripe)`, "restaurant");
  return orderId;
}
var APP2, n5, cents, remainingEur;
var init_payments = __esm({
  "apps/api/src/lib/payments.ts"() {
    ...
    remainingEur = (o) => Math.max(0, Math.round((n5(o.totalEur) + n5(o.deliveryFeeEur) - n5(o.paidAmountEur)) * 100) / 100);
  }
});
...
import { and as and12, desc as desc3, eq as eq14, ilike, isNull as isNull6, sql as sql8 } from "drizzle-orm";
...
var init_public = __esm({
  "apps/api/src/routes/public.ts"() {
    ...
    publicRoutes.get("/admin/leads", requireAuth, async (c) => {
      ...
      const [{ total }] = await db.select({ total: sql8`count(*)` }).from(leads);
    ...
    publicRoutes.put("/admin/leads/:id", requireAuth, async (c) => {
      ...
      if (!body3.success) return c.json({ error: "Donn\xE9es invalides" }, 400);
      const db = await getDb();
      const [row] = await db.update(leads).set(body3.data).where(eq14(leads.id, c.req.param("id") ?? "")).returning();
    ...
    publicRoutes.get("/public/reference", async (c) => {
      ...
      const db = await getDb();
      const rows = await db.select({ id: products.id, name: products.name, category: products.category, baseUnit: products.baseUnit }).from(products).where(and12(isNull6(products.restaurantId), ilike(products.name, `%${q2}%`))).orderBy(products.name).limit(20);
      return c.json({ products: rows });
    });
  }
});
...
import { and as and13, eq as eq15, sql as sql9, desc as desc4 } from "drizzle-orm";
async function invoiceCommissions(period, opts = {}) {
  const db = await getDb();
  const rows = await db.select({ vendorId: commissions.vendorId, orders: sql9`count(*)`, base: sql9`sum(${commissions.orderTotalEur})`, amount: sql9`sum(${commissions.amountEur})` }).from(commissions).where(and13(eq15(commissions.period, period), eq15(commissions.invoiced, false))).groupBy(commissions.vendorId);
  const out = [];
  for (const r of rows) {
    const amount = Number(r.amount);
    const [v] = await db.select().from(vendors).where(eq15(vendors.id, r.vendorId));
    if (!v || amount < 1) {
    ...
    const [existing] = await db.select().from(commissionInvoices).where(and13(eq15(commissionInvoices.vendorId, v.id), eq15(commissionInvoices.period, period)));
    ...
    if (via === "mail") {
      const [m] = await db.select({ email: users.email }).from(vendorMembers).innerJoin(users, eq15(users.id, vendorMembers.userId)).where(eq15(vendorMembers.vendorId, v.id)).limit(1);
...
L'\xE9quipe AFRISUPPLY`, html: `<p>Bonjour,</p><p>Relev\xE9 de commission <b>${period}</b> pour <b>${v.name}</b> :</p><ul><li>${r.orders} commande(s) confirm\xE9e(s), base ${eur7(Number(r.base))}</li><li>Commission ${Number(v.commissionPct)} % : <b>${eur7(amount)}</b></li></ul><p>R\xE8glement sous 15 jours par virement \u2014 ou activez le pr\xE9l\xE8vement automatique dans votre espace fournisseur.</p><p>L'\xE9quipe AFRISUPPLY</p>`, tags: { type: "commission" } });
    ...
    await db.update(commissions).set({ invoiced: true }).where(and13(eq15(commissions.vendorId, v.id), eq15(commissions.period, period)));
...
var init_billing2 = __esm({
  "apps/api/src/routes/billing.ts"() {
    ...
    billingPublicRoutes.post("/billing/webhook", async (c) => {
      ...
      const db = await getDb();
      const [seen] = await db.select({ id: billingEvents.id }).from(billingEvents).where(eq15(billingEvents.id, evt.id));
      ...
      try {
        ...
        if ((evt.type === "checkout.session.completed" || evt.type === "checkout.session.async_payment_succeeded") && o.mode === "payment" && o.metadata?.kind === "order_payment") {
          ...
          await db.update(restaurants).set({ subscriptionStatus: "past_due" }).where(eq15(restaurants.stripeCustomerId, o.customer));
        } else if (evt.type === "invoice.paid" && typeof o.customer === "string" && o.subscription) {
          await db.update(restaurants).set({ subscriptionStatus: "active" }).where(eq15(restaurants.stripeCustomerId, o.customer));
          const [ci] = await db.select().from(commissionInvoices).where(eq15(commissionInvoices.stripeInvoiceId, String(o.id)));
          if (ci) await db.update(commissionInvoices).set({ status: "payee" }).where(eq15(commissionInvoices.id, ci.id));
        } else if (evt.type === "invoice.paid" && typeof o.id === "string") {
          await db.update(commissionInvoices).set({ status: "payee" }).where(eq15(commissionInvoices.stripeInvoiceId, o.id));
        }
    ...
    billingRoutes.get("/orders/:id/payment", async (c) => {
      const db = await getDb();
      const [o] = await db.select().from(orders).where(and13(eq15(orders.id, c.req.param("id")), eq15(orders.restaurantId, c.get("restaurantId"))));
      if (!o?.vendorId) return c.json({ error: "Introuvable" }, 404);
      const [v] = await db.select({ stripeAccountId: vendors.stripeAccountId, stripePayoutsEnabled: vendors.stripePayoutsEnabled }).from(vendors).where(eq15(vendors.id, o.vendorId));
      const av = await paymentAvailability(o, v);
    ...
    billingRoutes.get("/billing", async (c) => {
      const db = await getDb();
      const [r] = await db.select().from(restaurants).where(eq15(restaurants.id, c.get("restaurantId")));
      const s = accessState(r);
      return c.json({
        plan: r.plan,
        founder: r.founder,
    ...
    billingAdminRoutes.put("/admin/billing/restaurants/:id", async (c) => {
      ...
      const db = await getDb();
      const [r] = await db.select().from(restaurants).where(eq15(restaurants.id, c.req.param("id")));
      ...
      const [upd] = await db.update(restaurants).set(patch).where(eq15(restaurants.id, r.id)).returning();
...
import { and as and14, desc as desc5, eq as eq16, gte as gte3, sql as sql10 } from "drizzle-orm";
...
async function activityCounts(rid) {
  const db = await getDb();
  const one = async (q2) => n6((await q2)[0]?.c);
  const [recipesC, invC, supC, offC, salesC, ordersC, delivC, movC, alertsUnread] = await Promise.all([
    one(db.select({ c: sql10`count(*)` }).from(recipes).where(eq16(recipes.restaurantId, rid))),
    one(db.select({ c: sql10`count(*)` }).from(inventoryItems).where(and14(eq16(inventoryItems.restaurantId, rid), sql10`${inventoryItems.quantity} > 0`))),
    one(db.select({ c: sql10`count(*)` }).from(suppliers).where(eq16(suppliers.restaurantId, rid))),
    one(db.select({ c: sql10`count(*)` }).from(supplierOffers).innerJoin(suppliers, eq16(suppliers.id, supplierOffers.supplierId)).where(eq16(suppliers.restaurantId, rid))),
    one(db.select({ c: sql10`count(*)` }).from(sales).where(eq16(sales.restaurantId, rid))),
    one(db.select({ c: sql10`count(*)` }).from(orders).where(and14(eq16(orders.restaurantId, rid), sql10`${orders.status} <> 'brouillon'`))),
    one(db.select({ c: sql10`count(*)` }).from(deliveries).where(eq16(deliveries.restaurantId, rid))),
    one(db.select({ c: sql10`count(*)` }).from(stockMovements).where(eq16(stockMovements.restaurantId, rid))),
    one(db.select({ c: sql10`count(*)` }).from(alerts).where(and14(eq16(alerts.restaurantId, rid), eq16(alerts.isRead, false))))
  ]);
...
async function pilotHealth(rid, now = /* @__PURE__ */ new Date()) {
  const db = await getDb();
  const [r] = await db.select().from(restaurants).where(eq16(restaurants.id, rid));
  ...
  const [{ c: events7 }] = await db.select({ c: sql10`count(*)` }).from(usageEvents).where(and14(eq16(usageEvents.restaurantId, rid), gte3(usageEvents.at, since7)));
  const [{ c: sales7 }] = await db.select({ c: sql10`count(*)` }).from(sales).where(and14(eq16(sales.restaurantId, rid), gte3(sales.day, since7.toISOString().slice(0, 10))));
  const [{ c: mov7 }] = await db.select({ c: sql10`count(*)` }).from(stockMovements).where(and14(eq16(stockMovements.restaurantId, rid), gte3(stockMovements.createdAt, since7)));
  const [{ c: orders7 }] = await db.select({ c: sql10`count(*)` }).from(orders).where(and14(eq16(orders.restaurantId, rid), gte3(orders.createdAt, since7), sql10`${orders.status} <> 'brouillon'`));
  const members = await db.select({ email: users.email, fullName: users.fullName, lastLoginAt: users.lastLoginAt, role: restaurantMembers.role }).from(restaurantMembers).innerJoin(users, eq16(users.id, restaurantMembers.userId)).where(eq16(restaurantMembers.restaurantId, rid));
  ...
  const [nps] = await db.select({ score: feedback.score, at: feedback.createdAt }).from(feedback).where(and14(eq16(feedback.restaurantId, rid), eq16(feedback.kind, "nps"))).orderBy(desc5(feedback.createdAt)).limit(1);
  const [{ c: openFeedback }] = await db.select({ c: sql10`count(*)` }).from(feedback).where(and14(eq16(feedback.restaurantId, rid), eq16(feedback.status, "nouveau"), sql10`${feedback.kind} <> 'nps'`));
  const dayNumber = Math.floor((now.getTime() - r.createdAt.getTime()) / 864e5) + 1;
  const doneSteps = steps.filter((s) => s.done).length;
  const active7 = n6(sales7) + n6(mov7) + n6(orders7) > 0 || n6(events7) >= 5;
  let status = "vert";
  const reasons = [];
  ...
  if (n6(openFeedback) > 0) reasons.push(`${n6(openFeedback)} retour(s) sans r\xE9ponse`);
  return { id: r.id, name: r.name, city: r.city, founder: r.founder, plan: r.plan, subscriptionStatus: r.subscriptionStatus, trialEndsAt: r.trialEndsAt, createdAt: r.createdAt, dayNumber, status, reasons, checklist: { done: doneSteps, total: steps.length }, counts, week: { events: n6(events7), sales: n6(sales7), movements: n6(mov7), orders: n6(orders7) }, daysSinceLogin, nps: nps ? { score: nps.score, at: nps.at } : null, openFeedback: n6(openFeedback), members: members.map((m) => ({ email: m.email, fullName: m.fullName, role: m.role, lastLoginAt: m.lastLoginAt })) };
}
async function buildWeeklyPilotReport(now = /* @__PURE__ */ new Date()) {
  ...
  const since7 = new Date(now.getTime() - 7 * 864e5);
  const newFb = await db.select({ f: feedback, restaurantName: restaurants.name }).from(feedback).innerJoin(restaurants, eq16(restaurants.id, feedback.restaurantId)).where(gte3(feedback.createdAt, since7)).orderBy(desc5(feedback.createdAt));
...
var isAdmin2, APP3, n6, ONBOARDING_STEPS, pilotPublicRoutes, pilotRoutes, pilotAdminRoutes;
var init_pilots = __esm({
  "apps/api/src/routes/pilots.ts"() {
    ...
    pilotPublicRoutes.get("/public/invite/:code", async (c) => {
      ...
      const db = await getDb();
      const [lead] = await db.select().from(leads).where(eq16(leads.inviteCode, code));
    ...
    pilotRoutes.get("/onboarding/checklist", async (c) => {
      const rid = c.get("restaurantId");
      const db = await getDb();
      const [r] = await db.select({ onboardingDone: restaurants.onboardingDone, founder: restaurants.founder, createdAt: restaurants.createdAt }).from(restaurants).where(eq16(restaurants.id, rid));
    ...
    pilotRoutes.post("/onboarding/checklist/:step", async (c) => {
      ...
      const [r] = await db.select({ onboardingDone: restaurants.onboardingDone }).from(restaurants).where(eq16(restaurants.id, rid));
      const set = new Set(r.onboardingDone ?? []);
      set.add(step);
      await db.update(restaurants).set({ onboardingDone: [...set] }).where(eq16(restaurants.id, rid));
      return c.json({ ok: true });
    });
    pilotRoutes.post("/feedback", async (c) => {
      ...
      const [r] = await db.select({ name: restaurants.name }).from(restaurants).where(eq16(restaurants.id, rid));
...
${APP3()}/app/admin/pilotes`, html: `<p><b>${r.name}</b> (${u.email}) \u2014 page ${b.page ?? "?"}</p><p>${(b.message ?? "(sans message)").replace(/\n/g, "<br>")}</p><p><a href="${APP3()}/app/admin/pilotes">Cockpit pilotes</a></p>`, tags: { type: "feedback" } });
    ...
    pilotRoutes.get("/feedback/nps-due", async (c) => {
      const db = await getDb();
      const rid = c.get("restaurantId");
      const [r] = await db.select({ createdAt: restaurants.createdAt }).from(restaurants).where(eq16(restaurants.id, rid));
      ...
      const [last] = await db.select({ id: feedback.id }).from(feedback).where(and14(eq16(feedback.restaurantId, rid), eq16(feedback.kind, "nps"), gte3(feedback.createdAt, since))).limit(1);
    ...
    pilotAdminRoutes.get("/admin/pilots", async (c) => {
      ...
      const fb = await db.select({ f: feedback, restaurantName: restaurants.name }).from(feedback).innerJoin(restaurants, eq16(restaurants.id, feedback.restaurantId)).orderBy(desc5(feedback.createdAt)).limit(100);
      ...
      const invited = await db.select({ id: leads.id, restaurantName: leads.restaurantName, contactName: leads.contactName, email: leads.email, inviteCode: leads.inviteCode, invitedAt: leads.invitedAt, restaurantId: leads.restaurantId, status: leads.status }).from(leads).where(sql10`${leads.inviteCode} is not null`).orderBy(desc5(leads.invitedAt));
    ...
    pilotAdminRoutes.post("/admin/pilots/invite", async (c) => {
      ...
      const db = await getDb();
      let lead = b.leadId ? (await db.select().from(leads).where(eq16(leads.id, b.leadId)))[0] : void 0;
      if (!lead) {
      ...
      await db.update(leads).set({ inviteCode: code, invitedAt: /* @__PURE__ */ new Date(), status: "pilote" }).where(eq16(leads.id, lead.id));
...
L'\xE9quipe AFRISUPPLY`,
    ...
    pilotAdminRoutes.put("/admin/pilots/feedback/:id", async (c) => {
      ...
      const db = await getDb();
      const [row] = await db.update(feedback).set({ status }).where(eq16(feedback.id, c.req.param("id"))).returning();
...
import { and as and15, eq as eq17, inArray as inArray5, isNull as isNull7, or, gte as gte4 } from "drizzle-orm";
async function loadPricing(offers, restaurantId) {
  const db = await getDb();
  const ids = offers.map((o) => o.id);
  const vendorIds = [...new Set(offers.map((o) => o.vendorId))];
  const tiers = ids.length ? await db.select().from(vendorPriceTiers).where(inArray5(vendorPriceTiers.vendorOfferId, ids)).orderBy(vendorPriceTiers.minPacks) : [];
  const today2 = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
  const custom = restaurantId && vendorIds.length ? await db.select().from(vendorCustomerPrices).where(and15(eq17(vendorCustomerPrices.restaurantId, restaurantId), inArray5(vendorCustomerPrices.vendorId, vendorIds), or(isNull7(vendorCustomerPrices.validUntil), gte4(vendorCustomerPrices.validUntil, today2)))) : [];
  return { tiers, custom };
}
function priceFor(offer, packs, ctx) {
  const list = n7(offer.packPriceEur);
  const tiers = ctx.tiers.filter((t) => t.vendorOfferId === offer.id).map((t) => ({ minPacks: t.minPacks, packPriceEur: n7(t.packPriceEur) })).sort((a, b) => a.minPacks - b.minPacks);
  ...
  if (own?.packPriceEur) return { ...base, packPriceEur: Math.min(n7(own.packPriceEur), base.packPriceEur), source: "negocie", negotiated: true };
  if (own?.discountPct) return { ...base, packPriceEur: r2(Math.min(list * (1 - n7(own.discountPct) / 100), base.packPriceEur)), source: "negocie", negotiated: true };
  if (global?.discountPct) return { ...base, packPriceEur: r2(Math.min(list * (1 - n7(global.discountPct) / 100), base.packPriceEur)), source: "remise_client", negotiated: true };
  return base;
}
var n7, r2;
...
import { and as and16, eq as eq18, inArray as inArray6, sql as sql11 } from "drizzle-orm";
...
async function nextSlots(vendorId, rest, days2 = 14, now = /* @__PURE__ */ new Date()) {
  const db = await getDb();
  const rs = (await db.select().from(vendorRoutes).where(and16(eq18(vendorRoutes.vendorId, vendorId), eq18(vendorRoutes.active, true)))).filter((r) => routeServes(r, rest));
  if (!rs.length) return [];
  const today2 = parisDate(now);
  const horizon = addDays(today2, days2);
  const load = await db.select({ routeId: orders.routeId, date: orders.expectedAt, count: sql11`count(*)` }).from(orders).where(and16(inArray6(orders.routeId, rs.map((r) => r.id)), sql11`${orders.expectedAt} between ${today2} and ${horizon}`, sql11`${orders.status} not in ('annulee')`)).groupBy(orders.routeId, orders.expectedAt);
  const out = [];
...
import { and as and17, gte as gte5, inArray as inArray7, sql as sql12 } from "drizzle-orm";
...
import { and as and18, desc as desc6, eq as eq20, gte as gte6, inArray as inArray8, sql as sql13 } from "drizzle-orm";
...
async function linkVendor(rid, vid) {
  const db = await getDb();
  const [v] = await db.select().from(vendors).where(and18(eq20(vendors.id, vid), eq20(vendors.status, "actif")));
  if (!v) return null;
  let [sup] = await db.select().from(suppliers).where(and18(eq20(suppliers.restaurantId, rid), eq20(suppliers.vendorId, vid)));
  if (!sup) {
  ...
  const vo = await db.select().from(vendorOffers).where(and18(eq20(vendorOffers.vendorId, vid), eq20(vendorOffers.inStock, true)));
  let synced = 0;
  for (const o of vo) {
    ...
    await db.insert(priceHistory).values({ restaurantId: rid, offerId: offer.id, unitPriceEur: (n8(o.packPriceEur) / n8(o.packQty)).toFixed(4), source: "catalogue" });
    synced++;
  }
...
async function placeVendorOrder(rid, vid, userId, input) {
  ...
  const vo = await db.select().from(vendorOffers).where(and18(eq20(vendorOffers.vendorId, vid), inArray8(vendorOffers.id, input.lines.map((l) => l.vendorOfferId))));
  ...
  const linesData = input.lines.map((l) => {
    ...
    return { productId: o.productId, packLabel: o.packLabel, packs: l.packs, quantity: (l.packs * n8(o.packQty)).toFixed(3), unitPriceEur: (pp / n8(o.packQty)).toFixed(4), lineTotalEur: (l.packs * pp).toFixed(2) };
  });
  const total = linesData.reduce((a, l) => a + Number(l.lineTotalEur), 0);
  if (!input.skipMin && total < n8(v.minOrderEur)) return { ok: false, error: `Minimum de commande ${eur8(n8(v.minOrderEur))} chez ${v.name} (panier : ${eur8(total)})`, status: 400 };
  const credit = await checkCredit(vid, rid, total + n8(v.deliveryFeeEur), v.name);
  if (!credit.ok) return { ok: false, error: credit.error, status: credit.status };
  const paymentDays = credit.terms?.paymentDays ?? 0;
  const [rz] = await db.select({ city: restaurants.city, postalCode: restaurants.postalCode }).from(restaurants).where(eq20(restaurants.id, rid));
...
`, html: `<p>Bonjour,</p><p><b>${r.name}</b> vous passe commande via AFRISUPPLY :</p><ul>${linesData.map((l) => `<li>${l.packs} \xD7 ${l.packLabel} \u2014 ${eur8(Number(l.lineTotalEur))}</li>`).join("")}</ul><p><b>Total : ${eur8(total)}</b></p><p><a href="${APP_URL2()}/fournisseur/commandes">Confirmer ou refuser</a></p>`, tags: { type: "vendor_order" } });
...
async function runRecurringOrders(now = /* @__PURE__ */ new Date()) {
  ...
  const due = await db.select().from(recurringOrders).where(and18(eq20(recurringOrders.enabled, true), eq20(recurringOrders.nextRunOn, today2)));
  const out = [];
  for (const r of due) {
    ...
    if (res.ok) {
      ...
      if (r.mode === "confirm") {
        const rcpts = await db.select({ email: users.email }).from(restaurantMembers).innerJoin(users, eq20(users.id, restaurantMembers.userId)).where(and18(eq20(restaurantMembers.restaurantId, r.restaurantId), inArray8(restaurantMembers.role, ["owner", "manager"])));
        for (const x of rcpts) void sendMail({ to: x.email, subject: `\u2705 \xC0 valider : commande \xAB ${r.name} \xBB (${res.order.reference}) \u2014 ${eur8(res.total)}`, text: `Bonjour,

Votre commande r\xE9currente \xAB ${r.name} \xBB chez ${res.vendorName} est pr\xEAte (${eur8(res.total)}). Elle ne partira qu'apr\xE8s votre validation.

Valider ou modifier : ${APP_URL2()}/app/achats?validate=${res.order.id}

AFRISUPPLY`, html: `<p>Bonjour,</p><p>Votre commande r\xE9currente \xAB <b>${r.name}</b> \xBB chez ${res.vendorName} est pr\xEAte (<b>${eur8(res.total)}</b>). Elle ne partira qu'apr\xE8s votre validation.</p><p><a href="${APP_URL2()}/app/achats?validate=${res.order.id}" style="background:#0f766e;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none">Valider la commande</a></p>` });
      }
    } else {
...
var marketplaceRoutes, MAX_PACKS_PER_LINE2, MAX_ORDER_LINES, n8, eur8, servesZone, DAYS;
var init_marketplace = __esm({
  "apps/api/src/routes/marketplace.ts"() {
    ...
    marketplaceRoutes.get("/marketplace/vendors", async (c) => {
      ...
      const linked = new Map((await db.select({ vendorId: suppliers.vendorId, supplierId: suppliers.id }).from(suppliers).where(and18(eq20(suppliers.restaurantId, rid), sql13`${suppliers.vendorId} is not null`))).map((x) => [x.vendorId, x.supplierId]));
    ...
    marketplaceRoutes.get("/marketplace/vendors/:id", async (c) => {
      ...
      for (const o of await db.select({ productId: supplierOffers.productId, unit: sql13`min(${supplierOffers.packPriceEur} / ${supplierOffers.packQty})` }).from(supplierOffers).where(eq20(supplierOffers.restaurantId, rid)).groupBy(supplierOffers.productId)) myBest.set(o.productId, n8(o.unit));
      const mine = new Set((await db.select({ productId: inventoryItems.productId }).from(inventoryItems).where(eq20(inventoryItems.restaurantId, rid))).map((x) => x.productId));
      const [link] = await db.select({ id: suppliers.id }).from(suppliers).where(and18(eq20(suppliers.restaurantId, rid), eq20(suppliers.vendorId, vid)));
      ...
      const offers = rows.map(({ offer, product }) => {
        ...
        const unit2 = pi.packPriceEur / n8(offer.packQty);
      ...
      const gbs = await db.select().from(groupBuys).where(and18(eq20(groupBuys.vendorId, vid), eq20(groupBuys.status, "ouvert"), gte6(groupBuys.closesAt, /* @__PURE__ */ new Date())));
    ...
    marketplaceRoutes.post("/marketplace/vendors/:id/orders", async (c) => {
      ...
      return c.json({ order: res.order, message: `Commande ${res.order.reference} envoy\xE9e \xE0 ${res.vendorName} (${eur8(res.total)})${res.order.routeId ? ` \u2014 livraison pr\xE9vue le ${(/* @__PURE__ */ new Date(`${res.order.expectedAt}T12:00:00Z`)).toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long" })}${res.order.deliverySlot ? ` (${res.order.deliverySlot})` : ""}` : ""}. Vous serez pr\xE9venu d\xE8s confirmation.` }, 201);
    });
    marketplaceRoutes.post("/marketplace/orders/:id/send", async (c) => {
      const rid = c.get("restaurantId");
      const db = await getDb();
      const [o] = await db.select().from(orders).where(and18(eq20(orders.id, c.req.param("id")), eq20(orders.restaurantId, rid)));
      ...
      const [v] = await db.select().from(vendors).where(eq20(vendors.id, o.vendorId));
      if (!v || v.status !== "actif") return c.json({ error: "Fournisseur inactif" }, 400);
      const ls = await db.select().from(orderLines).where(eq20(orderLines.orderId, o.id));
      const [upd] = await db.update(orders).set({ status: "envoyee", sentAt: /* @__PURE__ */ new Date() }).where(and18(eq20(orders.id, o.id), eq20(orders.status, "preparee"))).returning();
      if (!upd) return c.json({ error: "Commande d\xE9j\xE0 envoy\xE9e" }, 409);
      await notifyVendorNewOrder(upd, ls.map((l) => ({ packs: n8(l.packs), packLabel: l.packLabel, lineTotalEur: String(l.lineTotalEur) })), v, rid, n8(upd.totalEur), "recurrente");
    ...
    marketplaceRoutes.get("/marketplace/vendors/:id/credit", async (c) => {
      const rid = c.get("restaurantId");
      const vid = c.req.param("id");
      const terms = await termsFor(vid, rid);
      const exposure = (await exposureFor(vid, [rid])).get(rid);
      return c.json({ terms: terms ? { paymentDays: terms.paymentDays, creditLimitEur: terms.creditLimitEur === null ? null : n8(terms.creditLimitEur), blocked: terms.blocked } : { paymentDays: 0, creditLimitEur: null, blocked: false }, exposure });
    });
    marketplaceRoutes.get("/marketplace/payables", async (c) => {
      const rid = c.get("restaurantId");
      const db = await getDb();
      const t = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
      const rows = await db.select({ id: orders.id, reference: orders.reference, vendorId: orders.vendorId, vendorName: vendors.name, totalEur: orders.totalEur, feeEur: orders.deliveryFeeEur, paidAmountEur: orders.paidAmountEur, paidAt: orders.paidAt, dueAt: orders.dueAt, paymentDays: orders.paymentDays, status: orders.status, deliveredAt: sql13`coalesce(${orders.deliveredAt}, ${orders.vendorDeliveredAt})` }).from(orders).innerJoin(vendors, eq20(vendors.id, orders.vendorId)).where(and18(eq20(orders.restaurantId, rid), sql13`(${orders.status} in ('livree','livree_partiel') or ${orders.vendorDeliveredAt} is not null)`, sql13`${orders.paidAt} is null`)).orderBy(orders.dueAt);
      const items = rows.map((r) => {
        const due = Math.round((n8(r.totalEur) + n8(r.feeEur) - n8(r.paidAmountEur)) * 100) / 100;
    ...
    marketplaceRoutes.get("/marketplace/vendors/:id/slots", async (c) => {
      const rid = c.get("restaurantId");
      const db = await getDb();
      const [r] = await db.select({ city: restaurants.city, postalCode: restaurants.postalCode }).from(restaurants).where(eq20(restaurants.id, rid));
    ...
    marketplaceRoutes.post("/marketplace/vendors/:id/quote", async (c) => {
      ...
      if (!body3.success) return c.json({ error: "Donn\xE9es invalides" }, 400);
      const vo = await db.select().from(vendorOffers).where(and18(eq20(vendorOffers.vendorId, vid), inArray8(vendorOffers.id, body3.data.lines.map((l) => l.vendorOfferId))));
      const pricing = await loadPricing(vo, rid);
    ...
    marketplaceRoutes.get("/orders/:id/reorder-preview", async (c) => {
      ...
      const items = lines.map(({ l, productName }) => {
        ...
        return { productName, packLabel: off?.packLabel ?? l.packLabel, packs: n8(l.packs), vendorOfferId: off?.id ?? null, available: !!off?.inStock, oldPackPrice: Math.round(n8(l.lineTotalEur) / Math.max(1, n8(l.packs)) * 100) / 100, newPackPrice: off ? n8(off.packPriceEur) : null };
      });
      const total = items.reduce((a, i) => a + (i.available && i.newPackPrice !== null ? i.packs * i.newPackPrice : 0), 0);
      return c.json({ vendor: { id: o.vendorId, ...v }, items, total: Math.round(total * 100) / 100, oldTotal: n8(o.totalEur) });
    });
    marketplaceRoutes.post("/orders/:id/reorder", async (c) => {
      ...
      if (!lines) {
        ...
        const offers = await db.select().from(vendorOffers).where(and18(eq20(vendorOffers.vendorId, o.vendorId), eq20(vendorOffers.inStock, true)));
        lines = prev.map((l) => {
          const off = offers.find((x) => x.productId === l.productId && x.packLabel === l.packLabel) ?? offers.find((x) => x.productId === l.productId);
          return off ? { vendorOfferId: off.id, packs: n8(l.packs) } : null;
        }).filter((x) => !!x);
      }
    ...
    marketplaceRoutes.get("/recurring", async (c) => {
      ...
      return c.json({ recurring: rows.map(({ r, vendorName }) => ({ ...r, vendorName, daysLabel: r.weekdays.map((d) => DAYS[d]).join(", "), lines: r.lines.map((l) => {
        ...
        return { ...l, productName: o?.productName ?? "?", packLabel: o?.o.packLabel ?? null, packPriceEur: o ? n8(o.o.packPriceEur) : null, available: !!o?.o.inStock };
      }), estimatedTotal: Math.round(r.lines.reduce((a, l) => {
        const o = offers.find((x) => x.o.id === l.vendorOfferId);
        return a + (o ? l.packs * n8(o.o.packPriceEur) : 0);
      }, 0) * 100) / 100 })) });
    });
    marketplaceRoutes.post("/recurring", async (c) => {
      ...
      if (body3.data.fromOrderId) {
        const [o] = await db.select().from(orders).where(and18(eq20(orders.id, body3.data.fromOrderId), eq20(orders.restaurantId, rid)));
    ...
    marketplaceRoutes.put("/recurring/:id", async (c) => {
      ...
      const [row] = await db.update(recurringOrders).set(patch).where(and18(eq20(recurringOrders.id, c.req.param("id")), eq20(recurringOrders.restaurantId, rid))).returning();
    ...
    marketplaceRoutes.delete("/recurring/:id", async (c) => {
      const rid = c.get("restaurantId");
      const db = await getDb();
      const [row] = await db.delete(recurringOrders).where(and18(eq20(recurringOrders.id, c.req.param("id")), eq20(recurringOrders.restaurantId, rid))).returning();
      if (!row) return c.json({ error: "Introuvable" }, 404);
      return c.json({ ok: true });
    });
    marketplaceRoutes.post("/recurring/:id/run", async (c) => {
      ...
      const user = c.get("user");
      const db = await getDb();
      const [r] = await db.select().from(recurringOrders).where(and18(eq20(recurringOrders.id, c.req.param("id")), eq20(recurringOrders.restaurantId, rid)));
    ...
    marketplaceRoutes.post("/marketplace/group-buys/:id/join", async (c) => {
      ...
      if (body3.data.packs === 0) {
        await db.delete(groupBuyParticipations).where(and18(eq20(groupBuyParticipations.groupBuyId, id), eq20(groupBuyParticipations.restaurantId, rid)));
      ...
      if (n8(total) >= gb.targetPacks && gb.status === "ouvert") await db.update(groupBuys).set({ status: "atteint" }).where(eq20(groupBuys.id, id));
      return c.json({ ok: true, myPacks: body3.data.packs, committedPacks: n8(total), reached: n8(total) >= gb.targetPacks });
    });
    marketplaceRoutes.get("/marketplace/my-orders", async (c) => {
      const rid = c.get("restaurantId");
      const db = await getDb();
      const rows = await db.select({ order: orders, vendorName: vendors.name }).from(orders).innerJoin(vendors, eq20(vendors.id, orders.vendorId)).where(eq20(orders.restaurantId, rid)).orderBy(desc6(orders.createdAt)).limit(30);
...
import { and as and19, desc as desc7, eq as eq21, gte as gte7, inArray as inArray9, sql as sql14 } from "drizzle-orm";
async function buildDigestForRestaurant(rid, opts = {}) {
  ...
  const recentAlerts = await db.select().from(alerts).where(and19(eq21(alerts.restaurantId, rid), eq21(alerts.isRead, false), gte7(alerts.createdAt, since))).orderBy(desc7(alerts.createdAt)).limit(50);
  ...
  const pending = await db.select({ reference: orders.reference, supplierName: suppliers.name, status: orders.status, expectedAt: orders.expectedAt }).from(orders).innerJoin(suppliers, eq21(suppliers.id, orders.supplierId)).where(and19(eq21(orders.restaurantId, rid), inArray9(orders.status, ["envoyee", "confirmee"]))).orderBy(orders.expectedAt).limit(10);
  const yesterday = new Date(now.getTime() - 864e5).toISOString().slice(0, 10);
  const [ys] = await db.select({ p: sql14`coalesce(sum(${sales.portions}), 0)`, c: sql14`count(*)` }).from(sales).where(and19(eq21(sales.restaurantId, rid), eq21(sales.day, yesterday)));
  ...
  }).from(orders).where(and19(eq21(orders.restaurantId, rid), sql14`${orders.status} <> 'annulee'`));
  const evolutionPct = n9(sp.prev30) > 0 ? Math.round((n9(sp.last30) - n9(sp.prev30)) / n9(sp.prev30) * 1e3) / 10 : null;
  ...
  const input = {
    ...
    discrepancies: { count: n9(disc.count), openValue: Math.round(n9(disc.value) * 100) / 100 },
    pendingOrders: pending,
    salesYesterday: n9(ys.c) > 0 ? n9(ys.p) : null,
    spend: { thisMonth: n9(sp.thisMonth), evolutionPct }
  };
  return { input, ctx };
}
async function recipientsFor(rid, settingsRecipients) {
  ...
  const db = await getDb();
  const rows = await db.select({ email: users.email, fullName: users.fullName, role: restaurantMembers.role }).from(restaurantMembers).innerJoin(users, eq21(users.id, restaurantMembers.userId)).where(eq21(restaurantMembers.restaurantId, rid));
...
import { and as and22, eq as eq24, desc as desc10, gte as gte8, inArray as inArray11, sql as sql17 } from "drizzle-orm";
...
import { and as and21, desc as desc9, eq as eq23, inArray as inArray10, sql as sql16 } from "drizzle-orm";
...
function detectKind(text2, fallback = "vente") {
  const n17 = normalize2(text2);
  for (const [re, k] of KIND_WORDS) if (re.test(n17)) return k;
  return fallback;
}
...
function tokenize(text2) {
  let n17 = normalize2(text2);
  for (const [w, v] of Object.entries(NUM_WORDS)) n17 = n17.replace(new RegExp(`\\b${w}\\b`, "g"), String(v));
  n17 = n17.replace(/(\d)(kg|g|l|ml|cl)\b/g, "$1 $2");
  const out = [];
...
import { and as and20, desc as desc8, eq as eq22, ilike as ilike2, or as or2, sql as sql15 } from "drizzle-orm";
...
prospectRoutes.get("/admin/prospects", async (c) => {
  ...
  const where = and20(kind ? eq22(prospects.kind, kind) : void 0, status ? eq22(prospects.status, status) : void 0, q2 ? or2(ilike2(prospects.name, `%${q2}%`), ilike2(prospects.city, `%${q2}%`), ilike2(prospects.email, `%${q2}%`), ilike2(prospects.phone, `%${q2}%`), ilike2(prospects.contactName, `%${q2}%`)) : void 0);
...
function orderPdf(d) {
  ...
  for (const l of d.lines) {
    p.down(16);
    p.row([{ text: l.productName.slice(0, 42), x: cols[0].x, w: cols[0].w }, { text: (l.packLabel ?? "").slice(0, 20), x: cols[1].x, w: cols[1].w }, { text: String(l.packs), x: cols[2].x, w: cols[2].w, align: "right" }, { text: `${Number.isInteger(l.quantity) ? l.quantity : l.quantity.toFixed(2)} ${l.unit}`, x: cols[3].x, w: cols[3].w, align: "right" }, { text: eur9(l.unitPriceEur), x: cols[4].x, w: cols[4].w, align: "right" }, { text: eur9(l.lineTotalEur), x: cols[5].x, w: cols[5].w, align: "right" }]);
    p.line(m, p.cursor - 5, m + W, p.cursor - 5, 0.9);
  }
  p.down(18);
  if (d.deliveryFeeEur) {
    p.text("Frais de livraison HT", m + 300, 9, { align: "right", width: 160 });
    p.text(eur9(d.deliveryFeeEur), cols[5].x, 9, { align: "right", width: cols[5].w });
    p.down(14);
  }
  p.text("TOTAL HT", m + 300, 11, { bold: true, align: "right", width: 160 });
  p.text(eur9(d.totalEur + d.deliveryFeeEur), cols[5].x, 11, { bold: true, align: "right", width: cols[5].w });
  p.down(14);
...
function parseCatalogLine(rawIn) {
  ...
  let n17 = normalize2(raw.replace(/€|eur|euros|ttc|ht/gi, " "));
  const priceM = n17.match(/(\d+(?:\.\d+)?)\s*$/);
  if (!priceM) return null;
  const price = Number(priceM[1]);
  n17 = n17.slice(0, priceM.index).trim();
  if (!(price > 0)) return null;
  let packQty = 0;
  let packUnit = "";
  let packLabel = "";
  const multi = n17.match(/(\d+(?:\.\d+)?)\s*(kg|g|l|ml|cl)?\s*x\s*(\d+(?:\.\d+)?)\s*(kg|g|l|ml|cl)?/);
  if (multi) {
    ...
    n17 = n17.replace(multi[0], " ");
  } else {
    const q2 = n17.match(/(\d+(?:\.\d+)?)\s*(kg|kilos?|kgs|g|gr|l|lt|litres?|ml|cl|pcs?|pieces?|unites?|u|bottes?)\b/);
    if (q2) {
      packQty = Number(q2[1]);
      packUnit = UNIT[q2[2]] ?? "kg";
      const pw = n17.match(PACK_WORDS);
      packLabel = `${pw ? cap(pw[1]) + " " : ""}${q2[1]} ${packUnit}`;
      n17 = n17.replace(q2[0], " ").replace(PACK_WORDS, " ");
    } else {
      const pw = n17.match(PACK_WORDS);
      const cnt = n17.match(/\bx?\s*(\d+)\s*$/);
      packQty = cnt ? Number(cnt[1]) : 1;
      packUnit = "piece";
      packLabel = pw ? cap(pw[1]) : packQty > 1 ? `Lot de ${packQty}` : "Pi\xE8ce";
      n17 = n17.replace(PACK_WORDS, " ").replace(/\bx?\s*\d+\s*$/, " ");
    }
  }
  if (packUnit === "g") {
    packQty = packQty / 1e3;
  ...
  const label = n17.replace(/\s+/g, " ").replace(/[-–:]+$/, "").trim();
...
vendorRoutes2.get("/vendor/dashboard", async (c) => {
  ...
  const [{ commission }] = await db.select({ commission: sql16`coalesce(sum(${commissions.amountEur}),0)` }).from(commissions).where(and21(eq23(commissions.vendorId, vid), eq23(commissions.period, period)));
  return c.json({ vendor: v, stats: { pendingOrders: n10(stats.pending), confirmedOrders: n10(stats.confirmed), monthRevenue: n10(stats.month), restaurantsServed: n10(stats.restaurants), offers: n10(offers), restaurantsFollowing: n10(linked), commissionThisMonth: n10(commission), commissionPct: n10(v.commissionPct) } });
});
...
vendorRoutes2.get("/vendor/offers", async (c) => {
  ...
  return c.json({ offers: rows.map(({ offer, product }) => ({ ...offer, productName: product.name, category: product.category, unit: product.baseUnit, unitPrice: Math.round(n10(offer.packPriceEur) / n10(offer.packQty) * 1e4) / 1e4 })) });
});
vendorRoutes2.post("/vendor/offers", async (c) => {
  ...
  const vid = c.get("vendorId");
  const d = body3.data;
  const [p] = await db.select({ id: products.id }).from(products).where(and21(eq23(products.id, d.productId), sql16`${products.restaurantId} is null`));
...
Suivi : ${APP_URL2()}/app/achats`, kind, orderId, vendorId: o.vendorId, restaurantId: o.restaurantId });
  const rcpts = await db.select({ email: users.email }).from(restaurantMembers).innerJoin(users, eq23(users.id, restaurantMembers.userId)).where(and21(eq23(restaurantMembers.restaurantId, o.restaurantId), inArray10(restaurantMembers.role, ["owner", "manager"])));
...
vendorRoutes2.post("/vendor/orders/:id/confirm", async (c) => {
  ...
  const amount = n10(o.totalEur) * n10(v.commissionPct) / 100;
  ...
  void notifyRestaurant(o.id, `\u2705 ${v.name} a confirm\xE9 votre commande ${o.reference}`, `${v.name} a confirm\xE9 la commande ${o.reference} (${eur10(n10(o.totalEur))}).
...
vendorRoutes2.post("/vendor/orders/:id/propose", async (c) => {
  ...
  const offers = offerIds.length ? await db.select({ o: vendorOffers, productName: products.name }).from(vendorOffers).innerJoin(products, eq23(products.id, vendorOffers.productId)).where(and21(eq23(vendorOffers.vendorId, vid), inArray10(vendorOffers.id, offerIds))) : [];
  const out = [];
  for (const l of lines) {
    const ch = body3.data.lines.find((x) => x.lineId === l.l.id);
    const packs = n10(l.l.packs);
    const unitPack = n10(l.l.lineTotalEur) / Math.max(1, packs);
    if (!ch) {
      out.push({ lineId: l.l.id, productName: l.productName, packLabel: l.l.packLabel, packs, newPacks: packs, lineTotalEur: n10(l.l.lineTotalEur), newLineTotalEur: n10(l.l.lineTotalEur), replacement: null });
      continue;
    }
    ...
    if (ch.replacementOfferId) {
      ...
      replacement = { vendorOfferId: r.o.id, productId: r.o.productId, productName: r.productName, packLabel: r.o.packLabel, packQty: n10(r.o.packQty), packPriceEur: n10(r.o.packPriceEur), packs: rp, lineTotalEur: Math.round(rp * n10(r.o.packPriceEur) * 100) / 100 };
    }
    out.push({ lineId: l.l.id, productName: l.productName, packLabel: l.l.packLabel, packs, newPacks: ch.newPacks, lineTotalEur: n10(l.l.lineTotalEur), newLineTotalEur: Math.round(ch.newPacks * unitPack * 100) / 100, replacement });
  }
...
Proposition : ${summary}.
Nouveau total : ${eur10(newTotalEur)} (au lieu de ${eur10(n10(o.totalEur))}).${body3.data.note ? `
...
vendorRoutes2.get("/vendor/picking", async (c) => {
  ...
  for (const r of rows) {
    ...
    e.packs += n10(r.packs);
    e.orders.push({ reference: r.reference, restaurant: r.restaurant, packs: n10(r.packs) });
    byProduct.set(k, e);
  }
...
vendorRoutes2.get("/vendor/pricing", async (c) => {
  ...
  const customers = await db.select({ p: vendorCustomerPrices, restaurantName: restaurants.name, city: restaurants.city }).from(vendorCustomerPrices).innerJoin(restaurants, eq23(restaurants.id, vendorCustomerPrices.restaurantId)).where(eq23(vendorCustomerPrices.vendorId, vid)).orderBy(desc9(vendorCustomerPrices.updatedAt));
  ...
  return c.json({ tiers: tiers.map((x) => x.t), customers: customers.map((x) => ({ ...x.p, restaurantName: x.restaurantName, city: x.city })), clients: clients.map((x) => ({ ...x, orders: n10(x.orders), gmv: n10(x.gmv) })) });
});
vendorRoutes2.put("/vendor/offers/:id/tiers", async (c) => {
  ...
  for (let i = 0; i < sorted.length; i++) {
    if (sorted[i].packPriceEur >= n10(o.packPriceEur)) return c.json({ error: `Le palier ${sorted[i].minPacks}+ doit \xEAtre moins cher que le prix catalogue (${eur10(n10(o.packPriceEur))})` }, 400);
...
vendorRoutes2.post("/vendor/customer-prices", async (c) => {
  ...
  if (d.vendorOfferId) {
    ...
    if (!o) return c.json({ error: "Offre introuvable" }, 404);
    if (d.packPriceEur && d.packPriceEur >= n10(o.packPriceEur)) return c.json({ error: `Le prix n\xE9goci\xE9 doit \xEAtre inf\xE9rieur au catalogue (${eur10(n10(o.packPriceEur))})` }, 400);
  }
  ...
  const existing = await db.select().from(vendorCustomerPrices).where(and21(eq23(vendorCustomerPrices.vendorId, vid), eq23(vendorCustomerPrices.restaurantId, d.restaurantId), d.vendorOfferId ? eq23(vendorCustomerPrices.vendorOfferId, d.vendorOfferId) : sql16`${vendorCustomerPrices.vendorOfferId} is null`));
...
vendorRoutes2.post("/vendor/group-buys/:id/close", async (c) => {
  ...
  const packPrice = n10(offer.packPriceEur) * (1 - n10(gb.discountPct) / 100);
  ...
  for (const p of parts) {
    ...
    const [order] = await db.insert(orders).values({ restaurantId: p.restaurantId, supplierId: link.supplier.id, vendorId: vid, reference: await nextOrderReference(), status: "confirmee", channel: "plateforme", sentAt: /* @__PURE__ */ new Date(), vendorDecisionAt: /* @__PURE__ */ new Date(), expectedAt: gb.deliveryDate ?? new Date(Date.now() + v.leadTimeHours * 36e5).toISOString().slice(0, 10), totalEur: total.toFixed(2), deliveryFeeEur: "0", source: "achat_groupe", notes: `Achat group\xE9 \xAB ${gb.title} \xBB : \u2212${n10(gb.discountPct)} %` }).returning();
    await db.insert(orderLines).values({ orderId: order.id, productId: offer.productId, packLabel: offer.packLabel, packs: p.packs, quantity: (p.packs * n10(offer.packQty)).toFixed(3), unitPriceEur: (packPrice / n10(offer.packQty)).toFixed(4), lineTotalEur: total.toFixed(2) });
    await db.insert(commissions).values({ vendorId: vid, orderId: order.id, orderTotalEur: total.toFixed(2), pct: v.commissionPct, amountEur: (total * n10(v.commissionPct) / 100).toFixed(2), period: (/* @__PURE__ */ new Date()).toISOString().slice(0, 7) }).onConflictDoNothing();
    await db.update(groupBuyParticipations).set({ orderId: order.id }).where(eq23(groupBuyParticipations.id, p.id));
    void notifyRestaurant(order.id, `\u{1F91D} Achat group\xE9 r\xE9ussi : ${gb.title}`, `Le palier est atteint (${committed} colis). Votre commande ${order.reference} de ${p.packs} colis \xE0 ${eur10(packPrice)} le colis (\u2212${n10(gb.discountPct)} %) est confirm\xE9e chez ${v.name}.`);
    created++;
  }
...
vendorRoutes2.get("/vendor/commissions", async (c) => {
  const db = await getDb();
  const vid = c.get("vendorId");
  const rows = await db.select({ period: commissions.period, orders: sql16`count(*)`, base: sql16`sum(${commissions.orderTotalEur})`, amount: sql16`sum(${commissions.amountEur})`, invoiced: sql16`bool_and(${commissions.invoiced})` }).from(commissions).where(eq23(commissions.vendorId, vid)).groupBy(commissions.period).orderBy(desc9(commissions.period));
...
vendorAdminRoutes.get("/admin/vendors", async (c) => {
  ...
  const db = await getDb();
  const rows = await db.select({ v: vendors, offerCount: sql16`(select count(*) from vendor_offers o where o.vendor_id = ${vendors.id})`, orderCount: sql16`(select count(*) from orders o where o.vendor_id = ${vendors.id})` }).from(vendors).orderBy(desc9(vendors.createdAt));
...
Commission plateforme : ${n10(v.commissionPct)} % sur les commandes confirm\xE9es, factur\xE9e mensuellement.`, html: `<p>Votre espace <b>${v.name}</b> est actif : <a href="${APP_URL2()}/fournisseur">${APP_URL2()}/fournisseur</a></p><p>Commission plateforme : ${n10(v.commissionPct)} % sur les commandes confirm\xE9es.</p>`, tags: { type: "vendor_activated" } });
...
vendorRoutes2.post("/vendor/offers/quick", async (c) => {
  ...
  const what = parsed ? `${eur10(n10(row.offer.packPriceEur))} \u2192 ${eur10(parsed.price)}` : rupture ? "pass\xE9 en rupture" : "de nouveau disponible";
...
vendorRoutes2.get("/vendor/analytics", async (c) => {
  ...
  if (inZone.length) {
    ...
    const demand = await db.select({ productId: products.id, name: products.name, category: products.category, unit: products.baseUnit, restaurants: sql16`count(distinct ${inventoryItems.restaurantId})` }).from(inventoryItems).innerJoin(products, eq23(products.id, inventoryItems.productId)).where(and21(inArray10(inventoryItems.restaurantId, inZone), sql16`${products.restaurantId} is null`)).groupBy(products.id, products.name, products.category, products.baseUnit).orderBy(sql16`count(distinct ${inventoryItems.restaurantId}) desc`).limit(60);
    uncovered = demand.filter((d) => !mine.has(d.productId) && (v.categories.length === 0 || v.categories.includes(d.category))).slice(0, 20).map((d) => ({ ...d, restaurants: n10(d.restaurants) }));
  }
  const alerts2 = await db.select({ message: leads.message, c: sql16`count(*)` }).from(leads).where(and21(eq23(leads.source, "vitrine"), sql16`${leads.createdAt} >= now() - interval '90 days'`)).groupBy(leads.message).orderBy(sql16`count(*) desc`).limit(10);
  ...
  return c.json({
    ...
    alerts: alerts2.map((a) => ({ product: (a.message ?? "").replace(/^Alerte produit : /, ""), count: n10(a.c) }))
  });
});
async function buildOrderDoc(orderId, kind) {
  ...
  return { doc: orderPdf({
    ...
    lines: lines.map((x) => ({ productName: x.name, packLabel: x.l.packLabel, packs: x.l.packs, quantity: n10(x.l.quantity), unit: x.unit, unitPriceEur: n10(x.l.unitPriceEur), lineTotalEur: n10(x.l.lineTotalEur) })),
    totalEur: n10(o.order.totalEur),
    deliveryFeeEur: n10(o.order.deliveryFeeEur)
  }), order: o.order };
}
...
vendorRoutes2.get("/vendor/routes", async (c) => {
  const db = await getDb();
  const vid = c.get("vendorId");
  const rows = await db.select().from(vendorRoutes).where(eq23(vendorRoutes.vendorId, vid)).orderBy(vendorRoutes.weekday, vendorRoutes.name);
  const since = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
  const load = await db.select({ routeId: orders.routeId, date: orders.expectedAt, count: sql16`count(*)` }).from(orders).where(and21(eq23(orders.vendorId, vid), sql16`${orders.routeId} is not null`, sql16`${orders.expectedAt} >= ${since}`, sql16`${orders.status} not in ('annulee')`)).groupBy(orders.routeId, orders.expectedAt);
  return c.json({ routes: rows, upcoming: load.map((l) => ({ ...l, count: n10(l.count) })) });
});
...
vendorRoutes2.put("/vendor/routes/:id", async (c) => {
  ...
  const db = await getDb();
  const [r] = await db.update(vendorRoutes).set(body3.data).where(and21(eq23(vendorRoutes.id, c.req.param("id")), eq23(vendorRoutes.vendorId, c.get("vendorId")))).returning();
  if (!r) return c.json({ error: "Tourn\xE9e introuvable" }, 404);
  return c.json({ route: r });
});
vendorRoutes2.delete("/vendor/routes/:id", async (c) => {
  const db = await getDb();
  const [r] = await db.delete(vendorRoutes).where(and21(eq23(vendorRoutes.id, c.req.param("id")), eq23(vendorRoutes.vendorId, c.get("vendorId")))).returning();
  if (!r) return c.json({ error: "Tourn\xE9e introuvable" }, 404);
  return c.json({ ok: true });
});
vendorRoutes2.get("/vendor/credit", async (c) => {
  ...
  const vid = c.get("vendorId");
  const t = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
  const clients = await db.select({ id: restaurants.id, name: restaurants.name, city: restaurants.city }).from(orders).innerJoin(restaurants, eq23(restaurants.id, orders.restaurantId)).where(eq23(orders.vendorId, vid)).groupBy(restaurants.id, restaurants.name, restaurants.city);
  const terms = await db.select().from(vendorCreditTerms).where(eq23(vendorCreditTerms.vendorId, vid));
  ...
  if (extra.length) for (const r of await db.select({ id: restaurants.id, name: restaurants.name, city: restaurants.city }).from(restaurants).where(inArray10(restaurants.id, extra))) names.set(r.id, r);
  const rows = ids.map((id) => {
    const tm = terms.find((x) => x.restaurantId === id);
    const r = names.get(id);
    return { restaurantId: id, name: r.name, city: r.city, paymentDays: tm?.paymentDays ?? 0, creditLimitEur: tm?.creditLimitEur === null || tm?.creditLimitEur === void 0 ? null : n10(tm.creditLimitEur), blocked: tm?.blocked ?? false, note: tm?.note ?? null, ...exp.get(id) };
  }).sort((a, b) => b.outstandingEur - a.outstandingEur);
  const receivables = await db.select({ id: orders.id, reference: orders.reference, restaurantId: orders.restaurantId, restaurantName: restaurants.name, totalEur: orders.totalEur, feeEur: orders.deliveryFeeEur, paidAmountEur: orders.paidAmountEur, dueAt: orders.dueAt, paymentDays: orders.paymentDays, status: orders.status, deliveredAt: sql16`coalesce(${orders.deliveredAt}, ${orders.vendorDeliveredAt})` }).from(orders).innerJoin(restaurants, eq23(restaurants.id, orders.restaurantId)).where(and21(eq23(orders.vendorId, vid), sql16`(${orders.status} in ('livree','livree_partiel') or ${orders.vendorDeliveredAt} is not null)`, sql16`${orders.paidAt} is null`)).orderBy(orders.dueAt);
  const items = receivables.map((r) => ({ ...r, dueEur: Math.round((n10(r.totalEur) + n10(r.feeEur) - n10(r.paidAmountEur)) * 100) / 100, overdue: !!r.dueAt && r.dueAt < t })).filter((r) => r.dueEur > 0);
...
vendorRoutes2.put("/vendor/credit/:restaurantId", async (c) => {
  ...
  const db = await getDb();
  const vid = c.get("vendorId");
  const rid = c.req.param("restaurantId");
  const [r] = await db.select({ id: restaurants.id }).from(restaurants).where(eq23(restaurants.id, rid));
  if (!r) return c.json({ error: "Restaurant introuvable" }, 404);
...
vendorRoutes2.post("/vendor/orders/:id/payment", async (c) => {
  ...
  const total = Math.round((n10(o.totalEur) + n10(o.deliveryFeeEur)) * 100) / 100;
  const already = n10(o.paidAmountEur);
  ...
  const [upd] = await db.update(orders).set({ paidAmountEur: newPaid.toFixed(2), paidAt: settled ? body3.data.paidAt ? /* @__PURE__ */ new Date(`${body3.data.paidAt}T12:00:00Z`) : /* @__PURE__ */ new Date() : null, paymentMethod: body3.data.method }).where(eq23(orders.id, o.id)).returning();
...
vendorRoutes2.get("/vendor/payments", async (c) => {
  const db = await getDb();
  const vid = c.get("vendorId");
  const [v] = await db.select({ acct: vendors.stripeAccountId, enabled: vendors.stripePayoutsEnabled, commissionPct: vendors.commissionPct }).from(vendors).where(eq23(vendors.id, vid));
  if (!stripeConfigured()) return c.json({ configured: false, accountId: v.acct, payoutsEnabled: false, chargesEnabled: false, requirements: [], commissionPct: n10(v.commissionPct) });
  ...
  const online = await db.select({ count: sql16`count(*)`, sum: sql16`coalesce(sum(${orders.paidAmountEur}),0)` }).from(orders).where(and21(eq23(orders.vendorId, vid), eq23(orders.paymentMethod, "en_ligne")));
  return c.json({ configured: true, ...st, commissionPct: n10(v.commissionPct), onlinePayments: n10(online[0]?.count), onlineEur: n10(online[0]?.sum) });
});
...
manageRoutes.put("/suppliers/:id", async (c) => {
  ...
  }).where(and22(eq24(suppliers.id, c.req.param("id")), eq24(suppliers.restaurantId, rid))).returning();
...
manageRoutes.delete("/suppliers/:id", async (c) => {
  const rid = c.get("restaurantId");
  const db = await getDb();
  const [row] = await db.update(suppliers).set({ isActive: false }).where(and22(eq24(suppliers.id, c.req.param("id")), eq24(suppliers.restaurantId, rid))).returning({ id: suppliers.id });
  if (!row) return c.json({ error: "Fournisseur introuvable" }, 404);
...
manageRoutes.post("/suppliers/:id/offers", async (c) => {
  ...
  if (!body3.success) return c.json({ error: "Donn\xE9es invalides", details: body3.error.flatten() }, 400);
  const [sup] = await db.select({ id: suppliers.id }).from(suppliers).where(and22(eq24(suppliers.id, supplierId), eq24(suppliers.restaurantId, rid)));
  if (!sup) return c.json({ error: "Fournisseur introuvable" }, 404);
  const d = body3.data;
...
manageRoutes.put("/offers/:id", async (c) => {
  ...
  if (!body3.success) return c.json({ error: "Donn\xE9es invalides" }, 400);
  const d = body3.data;
  const [before] = await db.select().from(supplierOffers).where(and22(eq24(supplierOffers.id, c.req.param("id")), eq24(supplierOffers.restaurantId, rid)));
  ...
  const unitBefore = n11(before.packPriceEur) / n11(before.packQty), unitAfter = n11(offer.packPriceEur) / n11(offer.packQty);
...
manageRoutes.delete("/offers/:id", async (c) => {
  const rid = c.get("restaurantId");
  const db = await getDb();
  const [row] = await db.delete(supplierOffers).where(and22(eq24(supplierOffers.id, c.req.param("id")), eq24(supplierOffers.restaurantId, rid))).returning({ id: supplierOffers.id });
...
manageRoutes.get("/prices/:productId/history", async (c) => {
  ...
  for (const r of rows) {
    ...
    series.get(r.offerId).points.push({ at: r.recordedAt.toISOString(), price: n11(r.unitPrice), source: r.source });
  }
...
manageRoutes.put("/recipes/:id", async (c) => {
  ...
  }).where(and22(eq24(recipes.id, c.req.param("id")), eq24(recipes.restaurantId, rid))).returning();
...
manageRoutes.delete("/recipes/:id", async (c) => {
  const rid = c.get("restaurantId");
  const db = await getDb();
  const [r] = await db.delete(recipes).where(and22(eq24(recipes.id, c.req.param("id")), eq24(recipes.restaurantId, rid))).returning({ id: recipes.id });
  if (!r) return c.json({ error: "Recette introuvable" }, 404);
  return c.json({ ok: true });
});
manageRoutes.put("/stock/:itemId", async (c) => {
  ...
  }).where(and22(eq24(inventoryItems.id, c.req.param("itemId")), eq24(inventoryItems.restaurantId, rid))).returning();
...
manageRoutes.delete("/stock/:itemId", async (c) => {
  const rid = c.get("restaurantId");
  const db = await getDb();
  const [row] = await db.delete(inventoryItems).where(and22(eq24(inventoryItems.id, c.req.param("itemId")), eq24(inventoryItems.restaurantId, rid))).returning({ id: inventoryItems.id });
  if (!row) return c.json({ error: "Article introuvable" }, 404);
  return c.json({ ok: true });
});
manageRoutes.post("/stock/inventory", async (c) => {
  ...
  const items = await db.select().from(inventoryItems).where(and22(eq24(inventoryItems.restaurantId, rid), inArray11(inventoryItems.id, ids)));
  const now = /* @__PURE__ */ new Date();
  let adjusted = 0;
  let totalDelta = 0;
  for (const cnt of body3.data.counts) {
    ...
    if (!it) continue;
    const delta = cnt.quantity - n11(it.quantity);
...
manageRoutes.get("/orders/:id/message", async (c) => {
  ...
  const db = await getDb();
  const user = c.get("user");
  const [row] = await db.select({ order: orders, supplier: suppliers }).from(orders).innerJoin(suppliers, eq24(suppliers.id, orders.supplierId)).where(and22(eq24(orders.id, c.req.param("id")), eq24(orders.restaurantId, rid)));
  ...
  const msg = buildOrderMessage({
    ...
    total: n11(row.order.totalEur),
    deliveryFee: n11(row.order.deliveryFeeEur),
    lines: lines.map(({ line, productName, unit: unit2 }) => ({ productName, packLabel: line.packLabel, packs: line.packs, quantity: n11(line.quantity), unit: unit2, lineTotal: n11(line.lineTotalEur) }))
  });
  return c.json(msg);
});
manageRoutes.put("/orders/:id", async (c) => {
  ...
  if (!body3.success) return c.json({ error: "Donn\xE9es invalides" }, 400);
  const [cur] = await db.select().from(orders).where(and22(eq24(orders.id, c.req.param("id")), eq24(orders.restaurantId, rid)));
  if (!cur) return c.json({ error: "Commande introuvable" }, 404);
...
manageRoutes.put("/orders/:id/lines", async (c) => {
  ...
  const outOfRange = await assertPlausibleQuantity(rid, asked.filter(({ line }) => body3.data.lines.some((x) => x.lineId === line.id)).map(({ line, product }) => {
    ...
    const packQty = n11(line.quantity) / Math.max(1, line.packs) || 1;
  ...
  for (const l of lines) {
    ...
    const packQty = n11(l.quantity) / l.packs;
    const packPrice = n11(l.lineTotalEur) / l.packs;
  ...
  const [o] = await db.update(orders).set({ totalEur: n11(total).toFixed(2) }).where(eq24(orders.id, cur.id)).returning();
  return c.json({ order: o });
});
...
manageRoutes.post("/orders/:id/proposal", async (c) => {
  ...
  const rid = c.get("restaurantId");
  const user = c.get("user");
  const [o] = await db.select().from(orders).where(and22(eq24(orders.id, c.req.param("id")), eq24(orders.restaurantId, rid)));
  if (!o) return c.json({ error: "Commande introuvable" }, 404);
...
import { and as and23, eq as eq25, isNull as isNull8, sql as sql18 } from "drizzle-orm";
...
storefrontRoutes.get("/public/catalog", async (c) => {
  ...
  let items = rows.map((p) => {
    const b = bm.get(p.id);
    return { ...p, slug: slugify3(p.name), fromUnitPrice: b ? Number(Number(b.unit).toFixed(2)) : null, offerCount: b ? n12(b.offers) : 0, vendorCount: b ? n12(b.vendors) : 0 };
  });
...
storefrontRoutes.get("/public/products/:id", async (c) => {
  const db = await getDb();
  const id = c.req.param("id");
  const [p] = await db.select().from(products).where(and23(eq25(products.id, id), isNull8(products.restaurantId)));
  ...
  const offers = rows.map(({ o, v }) => ({
    ...
    packQty: n12(o.packQty),
    packPriceEur: n12(o.packPriceEur),
    unitPrice: Number((n12(o.packPriceEur) / n12(o.packQty)).toFixed(2)),
    inStock: o.inStock,
    updatedAt: o.updatedAt,
    vendor: { id: v.id, name: v.name, slug: v.slug, city: v.city, deliveryZones: v.deliveryZones, leadTimeHours: v.leadTimeHours, minOrderEur: n12(v.minOrderEur), deliveryFeeEur: n12(v.deliveryFeeEur) }
  })).sort((a, b) => Number(b.inStock) - Number(a.inStock) || a.unitPrice - b.unitPrice);
  const similar = await db.select({ id: products.id, name: products.name, category: products.category, baseUnit: products.baseUnit, origin: products.origin }).from(products).where(and23(isNull8(products.restaurantId), eq25(products.category, p.category), sql18`${products.id} <> ${id}`)).limit(8);
...
storefrontRoutes.get("/public/vendors", async (c) => {
  ...
  const cm = new Map(counts.map((x) => [x.vendorId, n12(x.offers)]));
  const rel = await reliabilityFor(vs.map((v) => v.id));
  return c.json({ vendors: vs.map((v) => ({ reliability: rel.get(v.id) ?? emptyReliability, id: v.id, name: v.name, slug: v.slug, description: v.description, city: v.city, deliveryZones: v.deliveryZones, categories: v.categories, leadTimeHours: v.leadTimeHours, minOrderEur: n12(v.minOrderEur), deliveryFeeEur: n12(v.deliveryFeeEur), offerCount: cm.get(v.id) ?? 0 })) });
});
...
import { and as and24, desc as desc11, eq as eq26, ilike as ilike3, isNull as isNull9, or as or3, sql as sql19 } from "drizzle-orm";
...
referenceAdminRoutes.get("/admin/reference", async (c) => {
  ...
  const q2 = (c.req.query("q") ?? "").trim();
  const cat = c.req.query("category");
  const where = and24(isNull9(products.restaurantId), cat ? eq26(products.category, cat) : void 0, q2 ? or3(ilike3(products.name, `%${q2}%`), sql19`exists (select 1 from unnest(${products.aliases}) a where a ilike ${"%" + q2 + "%"})`) : void 0);
  ...
  const requests = await db.select().from(leads).where(and24(eq26(leads.source, "referentiel"), eq26(leads.status, "nouveau"))).orderBy(desc11(leads.createdAt)).limit(50);
...
referenceAdminRoutes.post("/admin/reference", async (c) => {
  ...
  const db = await getDb();
  const d = b.data;
  const [dup] = await db.select({ id: products.id, name: products.name }).from(products).where(and24(isNull9(products.restaurantId), ilike3(products.name, d.name)));
...
referenceAdminRoutes.put("/admin/reference/:id", async (c) => {
  ...
  const db = await getDb();
  const d = b.data;
  const [p] = await db.update(products).set({ ...d, imageUrl: d.imageUrl === "" ? null : d.imageUrl }).where(and24(eq26(products.id, c.req.param("id")), isNull9(products.restaurantId))).returning();
  if (!p) return c.json({ error: "Produit introuvable" }, 404);
...
referenceRequestRoutes.post("/reference/request", requireAuth, async (c) => {
  ...
  const db = await getDb();
  const u = c.get("user");
  const [existing] = await db.select({ id: products.id, name: products.name }).from(products).where(and24(isNull9(products.restaurantId), or3(ilike3(products.name, b.data.product), sql19`exists (select 1 from unnest(${products.aliases}) a where lower(a) = lower(${b.data.product}))`)));
...
import { desc as desc12, eq as eq27, gte as gte9, isNull as isNull10, sql as sql20 } from "drizzle-orm";
...
adminDashboardRoutes.get("/admin/dashboard", async (c) => {
  ...
  const activeR = await db.select({ c: sql20`count(distinct ${orders.restaurantId})` }).from(orders).where(gte9(orders.createdAt, sql20`now() - interval '30 days'`));
  ...
  const weekly = await db.select({ week: sql20`to_char(date_trunc('week', ${orders.createdAt}), 'YYYY-MM-DD')`, all: sql20`count(*)`, mkt: sql20`count(*) filter (where ${orders.vendorId} is not null)`, gmv: sql20`coalesce(sum(${orders.totalEur}) filter (where ${orders.vendorId} is not null and ${orders.status} <> 'annulee'),0)` }).from(orders).where(gte9(orders.createdAt, sql20`date_trunc('week', now()) - interval '11 weeks'`)).groupBy(sql20`1`).orderBy(sql20`1`);
  const signups = await db.select({ week: sql20`to_char(date_trunc('week', ${restaurants.createdAt}), 'YYYY-MM-DD')`, c: sql20`count(*)` }).from(restaurants).where(gte9(restaurants.createdAt, sql20`date_trunc('week', now()) - interval '11 weeks'`)).groupBy(sql20`1`).orderBy(sql20`1`);
  ...
  const demandTop = await db.select({ name: products.name, restaurants: sql20`count(distinct ${inventoryItems.restaurantId})`, covered: sql20`exists (select 1 from vendor_offers o where o.product_id = ${products.id} and o.in_stock)` }).from(inventoryItems).innerJoin(products, eq27(products.id, inventoryItems.productId)).where(isNull10(products.restaurantId)).groupBy(products.id, products.name).orderBy(sql20`count(distinct ${inventoryItems.restaurantId}) desc`).limit(15);
  const topVendors = await db.select({ name: vendors.name, city: vendors.city, status: vendors.status, offers: sql20`(select count(*) from vendor_offers o where o.vendor_id = ${vendors.id})`, orders: sql20`(select count(*) from orders x where x.vendor_id = ${vendors.id})`, gmv: sql20`(select coalesce(sum(total_eur),0) from orders x where x.vendor_id = ${vendors.id} and x.status <> 'annulee')`, linked: sql20`(select count(*) from suppliers s where s.vendor_id = ${vendors.id})` }).from(vendors).orderBy(desc12(vendors.createdAt)).limit(10);
  ...
  const recentOrders = await db.select({ id: orders.id, reference: orders.reference, status: orders.status, total: orders.totalEur, createdAt: orders.createdAt, restaurant: restaurants.name, vendor: vendors.name }).from(orders).innerJoin(restaurants, eq27(restaurants.id, orders.restaurantId)).leftJoin(vendors, eq27(vendors.id, orders.vendorId)).orderBy(desc12(orders.createdAt)).limit(12);
  ...
  return c.json({
    users: num3(u),
    restaurants: { ...num3(r), active30: n13(activeR[0]?.c) },
    vendors: num3(v),
    offers: num3(vo),
    products: { ...num3(pr), tracked: n13(tracked.c) },
    orders: num3(o),
    ...
    env,
    todo: [
      ...n13(v.attente) ? [{ level: "action", text: `${n13(v.attente)} grossiste(s) en attente de validation`, to: "/app/admin/fournisseurs" }] : [],
      ...n13(ld.referentiel) ? [{ level: "action", text: `${n13(ld.referentiel)} demande(s) de produit manquant`, to: "/app/admin/referentiel" }] : [],
      ...n13(o.pending) ? [{ level: "watch", text: `${n13(o.pending)} commande(s) marketplace en attente de confirmation grossiste`, to: "/app/admin/fournisseurs" }] : [],
      ...!n13(v.actif) ? [{ level: "critical", text: "Aucun grossiste actif : la vitrine affiche \xAB Prix sur demande \xBB partout. Invitez vos 3 premiers grossistes.", to: "/app/admin/prospection" }] : [],
...
import { and as and26, desc as desc13, eq as eq28, sql as sql21 } from "drizzle-orm";
...
claimRoutes.get("/claims", async (c) => {
  const db = await getDb();
  const rid = c.get("restaurantId");
  const rows = await db.select({ cl: claims, vendorName: vendors.name, reference: orders.reference }).from(claims).innerJoin(vendors, eq28(vendors.id, claims.vendorId)).innerJoin(orders, eq28(orders.id, claims.orderId)).where(eq28(claims.restaurantId, rid)).orderBy(desc13(claims.createdAt)).limit(200);
  const items = rows.map((r) => ({ ...publicClaim(r.cl), vendorName: r.vendorName, orderReference: r.reference }));
  const credits = items.filter((i) => i.status === "accepte" || i.status === "clos").reduce((a, i) => a + n14(i.creditEur), 0);
...
claimRoutes.get("/claims/:id/photo", async (c) => {
  const db = await getDb();
  const rid = c.get("restaurantId");
  const [cl] = await db.select({ photo: claims.photo }).from(claims).where(and26(eq28(claims.id, c.req.param("id")), eq28(claims.restaurantId, rid)));
...
claimRoutes.post("/claims", async (c) => {
  ...
  if (d.discrepancyId) {
    ...
    orderedQty = n14(row.dd.orderedQty);
    receivedQty = n14(row.dd.receivedQty);
    if (claimedEur === void 0) claimedEur = Math.round(Math.max(0, orderedQty - receivedQty) * n14(row.unitPrice) * 100) / 100;
  }
  if (!orderId || !productName) return c.json({ error: "Commande et produit requis" }, 400);
  const [o] = await db.select().from(orders).where(and26(eq28(orders.id, orderId), eq28(orders.restaurantId, rid)));
  if (!o) return c.json({ error: "Commande introuvable" }, 404);
...
claimRoutes.post("/claims/:id/close", async (c) => {
  ...
  const db = await getDb();
  const rid = c.get("restaurantId");
  const [cl] = await db.select().from(claims).where(and26(eq28(claims.id, c.req.param("id")), eq28(claims.restaurantId, rid)));
  ...
  if (body3.data.action === "accept") {
    ...
    void logOrderEvent(cl.orderId, "note", `Litige ${cl.reference} clos \u2014 ${cl.resolution === "avoir" ? `avoir ${eur11(n14(cl.creditEur))}` : cl.resolution ?? ""}`, "restaurant");
...
[Escalade] ${body3.data.message}`.trim() : cl.message }).where(eq28(claims.id, cl.id)).returning();
  ...
  const admins = (process.env.ADMIN_EMAILS ?? "").split(",").map((s) => s.trim()).filter(Boolean);
  for (const to of admins) void sendMail({ to, subject: `\u{1F6A8} Litige escalad\xE9 ${cl.reference} (${eur11(n14(cl.claimedEur))})`, text: `Le restaurant conteste la r\xE9ponse du grossiste.
Litige : ${cl.reference} \u2014 ${cl.productName} \u2014 r\xE9clam\xE9 ${eur11(n14(cl.claimedEur))}, propos\xE9 ${cl.resolution ?? "\u2014"} ${cl.creditEur ? eur11(n14(cl.creditEur)) : ""}.
Message : ${body3.data.message ?? "\u2014"}
${APP_URL3()}/app/admin/litiges`, html: `<p>Litige <b>${cl.reference}</b> escalad\xE9 \u2014 ${cl.productName}, r\xE9clam\xE9 ${eur11(n14(cl.claimedEur))}.</p><p><a href="${APP_URL3()}/app/admin/litiges">Arbitrer</a></p>`, tags: { type: "claim" } });
...
vendorClaimRoutes.get("/vendor/claims", async (c) => {
  const db = await getDb();
  const vid = c.get("vendorId");
  const rows = await db.select({ cl: claims, restaurantName: restaurants.name, reference: orders.reference }).from(claims).innerJoin(restaurants, eq28(restaurants.id, claims.restaurantId)).innerJoin(orders, eq28(orders.id, claims.orderId)).where(eq28(claims.vendorId, vid)).orderBy(desc13(claims.createdAt)).limit(200);
  const items = rows.map((r) => ({ ...publicClaim(r.cl), photo: r.cl.photo, restaurantName: r.restaurantName, orderReference: r.reference }));
  return c.json({ items, openCount: items.filter((i) => i.status === "ouvert" || i.status === "escalade").length, creditsEur: Math.round(items.reduce((a, i) => a + n14(i.creditEur), 0) * 100) / 100 });
});
vendorClaimRoutes.post("/vendor/claims/:id/respond", async (c) => {
  ...
  const vid = c.get("vendorId");
  const d = body3.data;
  const [cl] = await db.select().from(claims).where(and26(eq28(claims.id, c.req.param("id")), eq28(claims.vendorId, vid)));
  if (!cl) return c.json({ error: "Litige introuvable" }, 404);
  ...
  const credit = d.resolution === "avoir" ? d.creditEur ?? n14(cl.claimedEur) : 0;
  if (d.resolution === "avoir" && credit <= 0) return c.json({ error: "Montant de l\u2019avoir requis" }, 400);
  const full = d.resolution === "avoir" && credit >= n14(cl.claimedEur) - 5e-3;
  ...
  const label = d.resolution === "avoir" ? `avoir de ${eur11(credit)}${full ? "" : ` (sur ${eur11(n14(cl.claimedEur))} r\xE9clam\xE9s)`}` : d.resolution === "relivraison" ? "relivraison du manquant" : `refus : ${d.message}`;
...
adminClaimRoutes.get("/admin/claims", async (c) => {
  const db = await getDb();
  const rows = await db.select({ cl: claims, restaurantName: restaurants.name, vendorName: vendors.name, reference: orders.reference }).from(claims).innerJoin(restaurants, eq28(restaurants.id, claims.restaurantId)).innerJoin(vendors, eq28(vendors.id, claims.vendorId)).innerJoin(orders, eq28(orders.id, claims.orderId)).orderBy(desc13(claims.createdAt)).limit(300);
...
import { and as and27, desc as desc14, eq as eq29, sql as sql22 } from "drizzle-orm";
...
publicReviewRoutes.get("/public/vendors/:id/reviews", async (c) => {
  const db = await getDb();
  const vid = c.req.param("id");
  const rows = await db.select({ r: vendorReviews, name: restaurants.name, city: restaurants.city }).from(vendorReviews).innerJoin(restaurants, eq29(restaurants.id, vendorReviews.restaurantId)).where(eq29(vendorReviews.vendorId, vid)).orderBy(desc14(vendorReviews.createdAt)).limit(50);
...
reviewRoutes.get("/reviews/pending", async (c) => {
  const db = await getDb();
  const rid = c.get("restaurantId");
  const rows = await db.select({ id: orders.id, reference: orders.reference, deliveredAt: sql22`coalesce(${orders.deliveredAt}, ${orders.vendorDeliveredAt})`, status: orders.status, vendorName: vendors.name, vendorId: vendors.id, reviewed: vendorReviews.id }).from(orders).innerJoin(vendors, eq29(vendors.id, orders.vendorId)).leftJoin(vendorReviews, eq29(vendorReviews.orderId, orders.id)).where(and27(eq29(orders.restaurantId, rid))).orderBy(desc14(sql22`coalesce(${orders.deliveredAt}, ${orders.vendorDeliveredAt}, ${orders.createdAt})`)).limit(100);
...
reviewRoutes.post("/orders/:id/review", async (c) => {
  ...
  const rid = c.get("restaurantId");
  const user = c.get("user");
  const [o] = await db.select().from(orders).where(and27(eq29(orders.id, c.req.param("id")), eq29(orders.restaurantId, rid)));
  if (!o?.vendorId) return c.json({ error: "Commande plateforme introuvable" }, 404);
...
vendorReviewRoutes.get("/vendor/reviews", async (c) => {
  const db = await getDb();
  const vid = c.get("vendorId");
  const rows = await db.select({ r: vendorReviews, name: restaurants.name, reference: orders.reference }).from(vendorReviews).innerJoin(restaurants, eq29(restaurants.id, vendorReviews.restaurantId)).innerJoin(orders, eq29(orders.id, vendorReviews.orderId)).where(eq29(vendorReviews.vendorId, vid)).orderBy(desc14(vendorReviews.createdAt)).limit(100);
...
vendorReviewRoutes.post("/vendor/reviews/:id/reply", async (c) => {
  ...
  const db = await getDb();
  const vid = c.get("vendorId");
  const [r] = await db.update(vendorReviews).set({ vendorReply: body3.data.reply, vendorRepliedAt: /* @__PURE__ */ new Date() }).where(and27(eq29(vendorReviews.id, c.req.param("id")), eq29(vendorReviews.vendorId, vid))).returning();
...
import { and as and28, eq as eq31, inArray as inArray12 } from "drizzle-orm";
...
accountRoutes.delete("/account", async (c) => {
  ...
  const left = [];
  for (const m of mine) {
    const others = await db.select({ id: restaurantMembers.userId }).from(restaurantMembers).where(and28(eq31(restaurantMembers.restaurantId, m.rid), eq31(restaurantMembers.role, "owner")));
...
import { and as and29, eq as eq32, ne, sql as sql23 } from "drizzle-orm";
...
var countOwners = async (rid) => {
  const db = await getDb();
  const [row] = await db.select({ n: sql23`count(*)` }).from(restaurantMembers).where(and29(eq32(restaurantMembers.restaurantId, rid), eq32(restaurantMembers.role, "owner")));
...
memberRoutes.post("/members", requireMinRole("owner"), async (c) => {
  ...
  const existing = await db.select({ role: restaurantMembers.role }).from(restaurantMembers).where(and29(eq32(restaurantMembers.restaurantId, rid), eq32(restaurantMembers.userId, user.id)));
...
memberRoutes.patch("/members/:userId", requireMinRole("owner"), async (c) => {
  ...
  const [row] = await db.select().from(restaurantMembers).where(and29(eq32(restaurantMembers.restaurantId, rid), eq32(restaurantMembers.userId, target)));
  ...
  await db.update(restaurantMembers).set({ role: body3.data.role }).where(and29(eq32(restaurantMembers.restaurantId, rid), eq32(restaurantMembers.userId, target)));
...
memberRoutes.delete("/members/:userId", requireMinRole("owner"), async (c) => {
  ...
  await db.delete(restaurantMembers).where(and29(eq32(restaurantMembers.restaurantId, rid), eq32(restaurantMembers.userId, target)));
  const [other] = await db.select({ n: sql23`count(*)` }).from(restaurantMembers).where(and29(eq32(restaurantMembers.userId, target), ne(restaurantMembers.restaurantId, rid)));
...
import { and as and30, eq as eq33, inArray as inArray13, sql as sql24 } from "drizzle-orm";
...
async function entities(rid) {
  ...
  const [recs, items] = await Promise.all([
    db.select({ id: recipes.id, name: recipes.name }).from(recipes).where(and30(eq33(recipes.restaurantId, rid), eq33(recipes.isActive, true))),
...
quickRoutes.post("/quick/apply", async (c) => {
  ...
  if (d.kind === "vente") {
    ...
    const valid = new Set((await db.select({ id: recipes.id }).from(recipes).where(and30(eq33(recipes.restaurantId, rid), inArray13(recipes.id, ids2)))).map((r) => r.id));
    const prev = new Map((await db.select().from(sales).where(and30(eq33(sales.restaurantId, rid), eq33(sales.day, day)))).map((p) => [p.recipeId, p.portions]));
    let applied2 = 0;
    let consumed = 0;
    for (const l of d.lines) {
      ...
      for (const ing of ings) {
        const [item] = await db.select().from(inventoryItems).where(and30(eq33(inventoryItems.restaurantId, rid), eq33(inventoryItems.productId, ing.productId)));
        if (!item) continue;
        const q2 = n15(ing.quantity) * delta;
        if (!q2) continue;
        ...
        await db.update(inventoryItems).set({ quantity: Math.max(0, n15(item.quantity) - q2).toFixed(3), updatedAt: /* @__PURE__ */ new Date() }).where(eq33(inventoryItems.id, item.id));
        consumed++;
      }
    }
  ...
  const items = await db.select().from(inventoryItems).where(and30(eq33(inventoryItems.restaurantId, rid), inArray13(inventoryItems.id, ids)));
  ...
  for (const l of d.lines) {
    ...
    const delta = type === "ajustement" ? l.qty - n15(item.quantity) : type === "reception" ? l.qty : -l.qty;
    ...
    await db.update(inventoryItems).set({ quantity: Math.max(0, n15(item.quantity) + delta).toFixed(3), updatedAt: /* @__PURE__ */ new Date(), ...type === "ajustement" ? { lastCountedAt: /* @__PURE__ */ new Date() } : {} }).where(eq33(inventoryItems.id, item.id));
    applied++;
  }
...
quickRoutes.get("/quick/inventory", async (c) => {
  ...
  const items = rows.map((r) => ({ ...r, quantity: n15(r.quantity), criticalLevel: n15(r.criticalLevel), daysSinceCount: r.lastCountedAt ? Math.floor((Date.now() - new Date(r.lastCountedAt).getTime()) / 864e5) : null })).sort((a, b) => (a.daysSinceCount ?? 999) === (b.daysSinceCount ?? 999) ? a.name.localeCompare(b.name) : (b.daysSinceCount ?? 999) - (a.daysSinceCount ?? 999));
  ...
  return c.json({ items, total: items.length, countedLast7Days: n15(counted7) });
});
...
quickRoutes.post("/quick/invoice/apply", async (c) => {
  ...
  const user = c.get("user");
  const d = body3.data;
  const items = await db.select().from(inventoryItems).where(and30(eq33(inventoryItems.restaurantId, rid), inArray13(inventoryItems.id, d.lines.map((l) => l.inventoryItemId))));
  const byId = new Map(items.map((i) => [i.id, i]));
  let received = 0;
  let pricesUpdated = 0;
  if (d.supplierId) {
    const [s] = await db.select({ id: suppliers.id }).from(suppliers).where(and30(eq33(suppliers.id, d.supplierId), eq33(suppliers.restaurantId, rid)));
    if (!s) return c.json({ error: "Fournisseur introuvable" }, 404);
  }
  for (const l of d.lines) {
    ...
    await db.update(inventoryItems).set({ quantity: (n15(item.quantity) + l.qty).toFixed(3), updatedAt: /* @__PURE__ */ new Date() }).where(eq33(inventoryItems.id, item.id));
    received++;
...
import { and as and31, desc as desc15, eq as eq34, inArray as inArray14, isNull as isNull11, or as or4, sql as sql25 } from "drizzle-orm";
init_marketplace();
...
shoppingRoutes.post("/shopping/parse", async (c) => {
  ...
  const zones = restaurantZones(r);
  const prods = await db.select().from(products).where(or4(isNull11(products.restaurantId), eq34(products.restaurantId, rid)));
  ...
  const vo = activeVendors.length ? await db.select().from(vendorOffers).where(and31(inArray14(vendorOffers.vendorId, activeVendors.map((v) => v.id)), eq34(vendorOffers.inStock, true))) : [];
  const mySups = await db.select().from(suppliers).where(and31(eq34(suppliers.restaurantId, rid), eq34(suppliers.isActive, true)));
  const sById = new Map(mySups.map((s) => [s.id, s]));
  const linkedVendorIds = new Set(mySups.map((s) => s.vendorId).filter(Boolean));
  const so = mySups.length ? await db.select().from(supplierOffers).where(and31(eq34(supplierOffers.restaurantId, rid), eq34(supplierOffers.inStock, true), inArray14(supplierOffers.supplierId, mySups.map((s) => s.id)))) : [];
  const lines = tokens.map((t) => {
    ...
    for (const o of vo.filter((x) => x.productId === p.id)) {
      ...
      const packs = asPacks ? Math.max(1, Math.round(needed)) : Math.max(1, Math.ceil(needed / n16(o.packQty)));
      offers.push({ key: `v:${o.id}`, kind: "vendor", offerId: o.id, sellerId: v.id, sellerName: v.name, packLabel: o.packLabel, packQty: n16(o.packQty), packPrice: n16(o.packPriceEur), unitPrice: n16(o.packPriceEur) / n16(o.packQty), leadTimeHours: v.leadTimeHours, minOrderEur: n16(v.minOrderEur), deliveryFeeEur: n16(v.deliveryFeeEur), packs, lineTotal: packs * n16(o.packPriceEur), linked: false });
    }
    for (const o of so.filter((x) => x.productId === p.id)) {
      const s = sById.get(o.supplierId);
      const packs = asPacks ? Math.max(1, Math.round(needed)) : Math.max(1, Math.ceil(needed / n16(o.packQty)));
      offers.push({ key: `s:${o.id}`, kind: "supplier", offerId: o.id, sellerId: s.id, sellerName: s.name, packLabel: o.packLabel, packQty: n16(o.packQty), packPrice: n16(o.packPriceEur), unitPrice: n16(o.packPriceEur) / n16(o.packQty), leadTimeHours: s.leadTimeHours, minOrderEur: n16(s.minOrderEur), deliveryFeeEur: n16(s.deliveryFeeEur), packs, lineTotal: packs * n16(o.packPriceEur), linked: true });
    }
...
shoppingRoutes.get("/shopping/lists", async (c) => {
  const rid = c.get("restaurantId");
  const db = await getDb();
  const lists = await db.select().from(shoppingLists).where(eq34(shoppingLists.restaurantId, rid)).orderBy(desc15(shoppingLists.lastUsedAt), desc15(shoppingLists.createdAt));
  return c.json({ lists });
});
...
shoppingRoutes.put("/shopping/lists/:id", async (c) => {
  ...
  const [list] = await db.update(shoppingLists).set(set).where(and31(eq34(shoppingLists.id, c.req.param("id")), eq34(shoppingLists.restaurantId, rid))).returning();
...
shoppingRoutes.delete("/shopping/lists/:id", async (c) => {
  const rid = c.get("restaurantId");
  const db = await getDb();
  const del = await db.delete(shoppingLists).where(and31(eq34(shoppingLists.id, c.req.param("id")), eq34(shoppingLists.restaurantId, rid))).returning({ id: shoppingLists.id });
...
shoppingRoutes.get("/shopping/suggestions", async (c) => {
  ...
  const low = inv.filter(({ item }) => n16(item.quantity) <= n16(item.criticalLevel) || item.targetLevel !== null && n16(item.quantity) < n16(item.targetLevel) * 0.5).map(({ item, product }) => {
    const target = item.targetLevel !== null ? n16(item.targetLevel) : Math.max(n16(item.criticalLevel) * 2, n16(item.avgDailyUse) * 7);
    const need = Math.max(0, target - n16(item.quantity));
    return { name: product.name, unit: product.baseUnit, need: Math.ceil(need * 10) / 10 };
  }).filter((x) => x.need > 0);
  const lastOrders = await db.select().from(orders).where(and31(eq34(orders.restaurantId, rid), inArray14(orders.status, ["envoyee", "confirmee", "livree", "livree_partiel"]))).orderBy(desc15(orders.createdAt)).limit(5);
  let last = null;
  if (lastOrders.length) {
    ...
    last = { reference: o.reference, date: o.createdAt.toISOString().slice(0, 10), text: ls.map(({ line, product }) => `${fmtQ(n16(line.quantity))} ${product.baseUnit} ${product.name}`).join(", ") };
  }
...
import { desc as desc16, eq as eq35, sql as sql26 } from "drizzle-orm";
...
statusRoutes.get("/status", async (c) => {
  ...
  try {
    ...
    dbMs = Date.now() - t0;
    dbOk = true;
    const [j] = await db.select().from(jobRuns).where(eq35(jobRuns.job, "daily")).orderBy(desc16(jobRuns.startedAt)).limit(1);
